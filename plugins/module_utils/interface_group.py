...
class PFSenseInterfaceGroupModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseInterfaceGroupModule, self).__init__(
            module,
            pfsense,
            root="ifgroups",
            create_root=True,
        ...
        if params["state"] == "present":
            ...
            for interface in params["members"]:
                if self.pfsense.is_interface_display_name(interface):
                    members.append(
                        self.pfsense.get_interface_by_display_name(interface)
                    )
                elif self.pfsense.is_interface_port(interface):
                    members.append(interface)
                else:
                    self.module.fail_json(
                        msg='Unknown interface name "{0}".'.format(interface)
                    )
    ...
    def _validate_params(self):
        ...
        if re.match("^[a-zA-Z0-9_]+$", params["name"]) is None:
            self.module.fail_json(
                msg='The name of the interface group may only consist of the characters "a-z, A-Z, 0-9 and _"'
            )
    ...
    def _update(self):
        ...
        return self.pfsense.phpshell(
            INTERFACE_GROUP_PHP_COMMAND.format(
                self.pfsense.dict_to_php(self.obj, "ifgroupentry")
            )
        )
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "descr", add_comma=(values), log_none=False
            )
            values += self.format_updated_cli_field(
                self.obj, before, "members", add_comma=(values)
            )
        return values
