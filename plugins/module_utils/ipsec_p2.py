...
IPSEC_P2_ARGUMENT_SPEC = dict(
    ...
    blowfish_len=dict(
        required=False, choices=["auto", "128", "192", "256"], type="str"
    ),
    # hashes
...
class PFSenseIpsecP2Module(PFSenseModuleBase):
    ...
    def _id_to_phase2(self, name, phase2, address, param_name):
        ...
        def set_ip_network():
            ...
            (phase2[name]["address"], phase2[name]["netbits"]) = (
                self.pfsense.parse_ip_network(address, False)
            )
            phase2[name]["netbits"] = str(phase2[name]["netbits"])

        phase2[name] = dict()

        interface = self.pfsense.parse_interface(
            address, fail=False, with_virtual=False
        )
        if interface is not None:
            ...
            if self.params["mode"] == "tunnel6":
                self.module.fail_json(
                    msg="A valid IPv6 address or network must be specified in {0} with tunnel6.".format(
                        param_name
                    )
                )
            set_ip_address()
            ...
            if self.params["mode"] == "tunnel":
                self.module.fail_json(
                    msg="A valid IPv4 address or network must be specified in {0} with tunnel.".format(
                        param_name
                    )
                )
            set_ip_address()
            ...
            self.module.fail_json(
                msg="A valid IP address, network or interface must be specified in {0}.".format(
                    param_name
                )
            )
    ...
    def _params_to_obj(self):
        ...
        if params["state"] == "present":
            ...
            if obj["mode"] != "transport":
                if obj["mode"] == "vti" and not self.pfsense.is_ipv4_address(
                    params["remote"]
                ):
    ...
    def _validate_params(self):
        ...
        if params.get("ikeid") is not None:
            ...
            if self._phase1 is None:
                self.module.fail_json(
                    msg="No ipsec tunnel with ikeid {0}".format(params["ikeid"])
                )
        else:
            self._phase1 = self.pfsense.find_ipsec_phase1(params["p1_descr"])
            if self._phase1 is None:
                self.module.fail_json(
                    msg="No ipsec tunnel named {0}".format(params["p1_descr"])
                )
        ...
        if params["state"] == "present":
            ...
            if need_one_hash and not has_one_of(
                ["sha1", "sha256", "sha384", "sha512", "aesxcbc"]
            ):
                self.module.fail_json(
        ...
        def sync_encryption(encryptions_elt, name, param_name):
            ...
            if self.params.get(param_name):
                ...
                if encryption_elt is None:
                    encryption_elt = self.pfsense.new_element(
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "sha1", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "sha256", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "sha384", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "sha512", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "aesxcbc", fvalue=self.fvalue_bool
            )
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "mode", add_comma=(values)
            )

            values += self.format_updated_cli_field(
                self.params, before, "local", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "remote", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "nat", add_comma=(values)
            )

            values += self.format_updated_cli_field(
                self.params, before, "aes", add_comma=(values), fvalue=self.fvalue_bool
            )
            values += self.format_updated_cli_field(
                self.params, before, "aes_len", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.params, before, "aes128gcm_len", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.params, before, "aes192gcm_len", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.params, before, "aes256gcm_len", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.params, before, "blowfish_len", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "des", add_comma=(values), fvalue=self.fvalue_bool
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.params, before, "sha1", add_comma=(values), fvalue=self.fvalue_bool
            )
            values += self.format_updated_cli_field(
                self.params,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "pfsgroup", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "lifetime", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "pinghost", add_comma=(values)
            )
        return values
    ...
    def _prepare_log_address(self, before, param, name):
        ...
        if (
            before.get(name) is None
            or not isinstance(before[name], dict)
            or before[name].get("type") is None
        ):
            before[param] = None
            return
        ...
        if before[name]["type"] == "address":
            ...
            before[param] = self.pfsense.get_interface_display_name(
                before[name]["type"]
            )
