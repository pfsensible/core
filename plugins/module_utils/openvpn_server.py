...
OPENVPN_SERVER_ARGUMENT_SPEC = dict(
    ...
    create_gw=dict(
        default="both", required=False, type="str", choices=["both", "v4only", "v6only"]
    ),
...
OPENVPN_SERVER_PHP_COMMAND_SET = (
    OPENVPN_SERVER_PHP_COMMAND_PREFIX
    + """
...
OPENVPN_SERVER_PHP_COMMAND_DEL = (
    OPENVPN_SERVER_PHP_COMMAND_PREFIX
    + """
...
class PFSenseOpenVPNServerModule(PFSenseModuleBase):
    ...
    def _params_to_obj(self):
        ...
        if self.params["state"] == "present":
            ...
            self._get_ansible_param_bool(
                obj, "ncp_enable", force=True, value="enabled", value_false="disabled"
            )
            ...
            self._get_ansible_param_bool(
                obj, "compression_push", force=True, value="yes", value_false=""
            )
            self._get_ansible_param_bool(
                obj, "passtos", force=True, value="yes", value_false=""
            )
            self._get_ansible_param_bool(
                obj, "client2client", force=True, value="yes", value_false=""
            )
            self._get_ansible_param_bool(
                obj, "dynamic_ip", force=True, value="yes", value_false=""
            )
            ...
            if "tls" in self.params["mode"]:
                ...
                if self.params is not None:
                    ...
                    if ca_elt is None:
                        self.module.fail_json(
                            msg="{0} is not a valid certificate authority".format(
                                self.params["ca"]
                            )
                        )
                ...
                if self.params["crl"] is not None:
                    ...
                    if crl_elt is None:
                        self.module.fail_json(
                            msg="{0} is not a valid certificate revocation list".format(
                                self.params["crl"]
                            )
                        )
                ...
                if self.params["cert"] is not None:
                    ...
                    if cert_elt is None:
                        self.module.fail_json(
                            msg="{0} is not a valid certificate".format(
                                self.params["cert"]
                            )
                        )
    ...
    def _validate_params(self):
        ...
        if len(params["authmode"]) > 0:
            ...
            for authsrv in params["authmode"]:
                if (
                    authsrv != "Local Database"
                    and len(system.findall("authserver[name='{0}']".format(authsrv)))
                    == 0
                ):
                    self.module.fail_json(
                        msg="Cannot find authentication server {0}.".format(authsrv)
                    )
    ...
    def _openvpn_port_used(self, protocol, interface, port, vpnid=0):
        for elt in self.root_elt.findall("*[local_port]"):
            ...
            if (
                (this_interface != "any" and interface != "any")
                and (len(protocol) == 4)
                and (len(this_protocol) == 4)
                and (this_protocol[0:3] == protocol[0:3])
                and (this_protocol[3] != protocol[3])
            ):
                continue
            ...
            if (
                ...
                and (
                    ...
                    or this_interface == "any"
                    or interface == "any"
                )
            ):
                self.module.fail_json(
                    msg="The specified local_port ({0}) is in use by vpn ID {1}".format(
                        port, this_vpnid
                    )
                )
    ...
    def _copy_and_update_target(self):
        ...
        (before, changed) = super(
            PFSenseOpenVPNServerModule, self
        )._copy_and_update_target()

        # Check if local port is used
        self._openvpn_port_used(
    ...
    def _create_target(self):
        ...
        # Check if local port is used
        self._openvpn_port_used(
            self.params["protocol"], self.params["interface"], self.params["local_port"]
        )
    ...
    def _pre_remove_target_elt(self):
        ...
        if (
            len(
                self.pfsense.interfaces.findall(
                    "*[if='ovpns{0}']".format(self.diff["before"]["vpnid"])
                )
            )
            > 0
        ):
            self.module.fail_json(
                msg="Cannot delete the OpenVPN instance while the interface ovpns{0} is assigned. Remove the interface assignment first.".format(
                    self.diff["before"]["vpnid"]
                )
            )

        self.result["vpnid"] = int(self.diff["before"]["vpnid"])
        self.command_output = self.pfsense.phpshell(
            OPENVPN_SERVER_PHP_COMMAND_DEL.format(idx=self.idx)
        )
    ...
    def _update(self):
        ...
        if self.params["state"] == "present":
            return self.pfsense.phpshell(
                OPENVPN_SERVER_PHP_COMMAND_SET.format(idx=self.idx)
            )
        else:
