    ...
    def _params_to_obj(self):
        ...
        if self.params["state"] == "present":
            ...
            self._get_ansible_param(
                obj, "check_frequency", fname="checkinter", force=True
            )
            self._get_ansible_param(obj, "retries", force=True)
            self._get_ansible_param_bool(
                obj, "log_checks", fname="log-health-checks", force=True
            )
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "balance_uriwhole", fvalue=self.fvalue_bool
            )
            ...
            values += self.format_cli_field(
                self.params, "log_checks", fvalue=self.fvalue_bool
            )
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "balance", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "balance_urilen", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "balance_uridepth", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "connection_timeout", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "server_timeout", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "check_type", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
                "checkinter",
                add_comma=(values),
                fname="check_frequency",
            )
            values += self.format_updated_cli_field(
                self.obj, before, "retries", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "httpcheck_method", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "monitor_uri", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "monitor_httpversion", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "monitor_username", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "monitor_domain", add_comma=(values)
            )
        return values
