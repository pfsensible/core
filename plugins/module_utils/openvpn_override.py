...
OPENVPN_OVERRIDE_ARGUMENT_SPEC = dict(
    ...
    netbios_ntype=dict(
        required=False, choices=["none", "b-node", "p-node", "m-node", "h-node"]
    ),
...
OPENVPN_OVERRIDE_PHP_COMMAND_SET = (
    OPENVPN_OVERRIDE_PHP_COMMAND_PREFIX
    + """
...
OPENVPN_OVERRIDE_PHP_COMMAND_DEL = (
    OPENVPN_OVERRIDE_PHP_COMMAND_PREFIX
    + """
...
class PFSenseOpenVPNOverrideModule(PFSenseModuleBase):
        ...
        if self.params["state"] == "present":
            ...
            if self.params["server_list"] is not None:
                for server in self.params["server_list"]:
                    vpnid = ""
                    if isinstance(server, int) or (
                        isinstance(server, str) and server.isdigit()
                    ):
                        openvpn_server_elt = self.pfsense.find_elt(
                            "openvpn-server",
                            str(server),
                            "vpnid",
                    ...
                    if openvpn_server_elt is None:
                        self.module.fail_json(
                            msg="Could not find openvpn server '%s'" % (server)
                        )
    ...
    def _validate_params(self):
        ...
        if params.get("local_network") and not self.pfsense.is_ipv4_network(
            params["local_network"]
        ):
            self.module.fail_json(
                msg="A valid IPv4 network must be specified for local_network."
            )
        if params.get("local_network6") and not self.pfsense.is_ipv6_network(
            params["local_networkv6"]
        ):
            self.module.fail_json(
                msg="A valid IPv6 network must be specified for local_network6."
            )
        if params.get("remote_network") and not self.pfsense.is_ipv4_network(
            params["remote_network"]
        ):
            self.module.fail_json(
                msg="A valid IPv4 network must be specified for remote_network."
            )
        if params.get("remote_network6") and not self.pfsense.is_ipv6_network(
            params["remote_networkv6"]
        ):
            self.module.fail_json(
                msg="A valid IPv6 network must be specified for remote_network6."
            )
    ...
    def _remove(self):
        ...
        return self.pfsense.phpshell(
            OPENVPN_OVERRIDE_PHP_COMMAND_DEL.format(idx=self.idx)
        )

    def _update(self):
        """make the target pfsense reload"""
        return self.pfsense.phpshell(
            OPENVPN_OVERRIDE_PHP_COMMAND_SET.format(idx=self.idx)
        )
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "descr", add_comma=(values)
            )
        return values
