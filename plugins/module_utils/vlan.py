...
class PFSenseVlanModule(PFSenseModuleBase):
    ...
    def _params_to_obj(self):
        ...
        if params["interface"] not in self.interfaces:
            obj["if"] = self.pfsense.get_interface_port_by_display_name(
                params["interface"]
            )
            if obj["if"] is None:
    ...
    def _validate_params(self):
        ...
        if params["interface"] not in self.interfaces:
            ...
            interface = self.pfsense.get_interface_port_by_display_name(
                params["interface"]
            )
            ...
            if interface is None or interface not in self.interfaces:
                self.module.fail_json(
                    msg="Vlans can't be set on interface {0}".format(
                        params["interface"]
                    )
                )
        ...
        if params["vlan_id"] < 1 or params["vlan_id"] > 4094:
            self.module.fail_json(
                msg="vlan_id must be between 1 and 4094 on interface {0}".format(
                    params["interface"]
                )
            )

        # check priority
        if params.get("priority") is not None and (
            params["priority"] < 0 or params["priority"] > 7
        ):
            self.module.fail_json(
                msg="priority must be between 0 and 7 on interface {0}".format(
                    params["interface"]
                )
            )
    ...
    def _cmd_create(self):
        ...
        cmd += (
            "if ($vlanif == NULL || $vlanif != $vlan['vlanif']) {pfSense_interface_destroy('%s');} else {\n"
            % (self.obj["vlanif"])
        )

        # if vlan is assigned to an interface, configuration needs to be applied again
        interface = self.pfsense.get_interface_by_port(
            "{0}.{1}".format(self.obj["if"], self.obj["tag"])
        )
    ...
    def _pre_remove_target_elt(self):
        ...
        if (
            self.pfsense.get_interface_by_port(
                "{0}.{1}".format(self.obj["if"], self.obj["tag"])
            )
            is not None
        ):
            self.module.fail_json(
                msg="vlan {0} on {1} cannot be deleted because it is still being used as an interface".format(
                    self.obj["tag"], self.obj["if"]
                )
            )
        self.setup_vlan_cmds += "pfSense_interface_destroy('{0}');\n".format(
            self.target_elt.find("vlanif").text
        )
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "pcp", add_comma=(values), fname="priority"
            )
            values += self.format_updated_cli_field(
