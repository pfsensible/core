...
class PFSenseRuleModule(PFSenseModuleBase):
    ...
    def _params_to_obj(self):
        ...
        if params["state"] == "present":
            ...
            if params["protocol"] not in ["tcp", "udp", "tcp/udp"] and (
                "port" in obj["source"] or "port" in obj["destination"]
            ):
                self.module.fail_json(
                    msg="{0}: you can't use ports on protocols other than tcp, udp or tcp/udp".format(
                        self._get_obj_name()
                    )
                )
            ...
            for param in ["destination", "source"]:
                if "address" in obj[param]:
                    self.pfsense.check_ip_address(
                        obj[param]["address"], obj["ipprotocol"], "rule"
                    )
    ...
    def _validate_params(self):
        ...
        if (
            params.get("ackqueue")
            and self.pfsense.find_queue(params["ackqueue"], enabled=True) is None
            and params["queue_error"]
        ):
            self.module.fail_json(
                msg="Failed to find enabled ackqueue=%s" % params["ackqueue"]
            )

        if (
            params.get("queue") is not None
            and self.pfsense.find_queue(params["queue"], enabled=True) is None
            and params["queue_error"]
        ):
            self.module.fail_json(
                msg="Failed to find enabled queue=%s" % params["queue"]
            )
        ...
        if (
            params.get("out_queue") is not None
            and params["out_queue"] == params["in_queue"]
        ):
            self.module.fail_json(msg="In and Out Queue cannot be the same")

        if (
            params.get("out_queue") is not None
            and self.pfsense.find_limiter(params["out_queue"], enabled=True) is None
        ):
            self.module.fail_json(
                msg="Failed to find enabled out_queue=%s" % params["out_queue"]
            )

        if (
            params.get("in_queue") is not None
            and self.pfsense.find_limiter(params["in_queue"], enabled=True) is None
        ):
            self.module.fail_json(
                msg="Failed to find enabled in_queue=%s" % params["in_queue"]
            )

        if (
            params.get("floating")
            and params.get("direction") == "any"
            and (params["in_queue"] is not None or params["out_queue"] is not None)
        ):
            self.module.fail_json(
        ...
        if params.get("gateway") is not None and params["gateway"] != "default":
            if params["ipprotocol"] == "inet46":
                self.module.fail_json(
                    msg='Gateway selection is not valid for "IPV4+IPV6" address family.'
                )
            elif (
                self.pfsense.find_gateway_group_elt(
                    params["gateway"], params["ipprotocol"]
                )
                is None
                and self.pfsense.find_gateway_elt(
                    params["gateway"], None, params["ipprotocol"]
                )
                is None
            ):
                self.module.fail_json(
                    msg='Gateway "%s" does not exist or does not match target rule ip protocol.'
                    % params["gateway"]
                )
        ...
        if params.get("tracker") is not None and int(params["tracker"]) < 0:
            self.module.fail_json(
                msg="tracker {0} must be a positive integer".format(params["tracker"])
            )

        # sched
        if (
            params.get("sched") is not None
            and self.pfsense.find_schedule_elt(params["sched"]) is None
        ):
            self.module.fail_json(
                msg="Schedule {0} does not exist".format(params["sched"])
            )
        ...
        if params.get("protocol") == "icmp" and params.get("icmptype") is not None:
            ...
            if len(invalids) > 0:
                self.module.fail_json(
                    msg="ICMP types {0} does not exist".format(",".join(invalids))
                )
            ...
            if len(left) > 0:
                self.module.fail_json(
                    msg="ICMP types {0} are invalid with IP type {1}".format(
                        ",".join(left), params["ipprotocol"]
                    )
                )
    ...
    def _copy_and_add_target(self):
        ...
        self.obj["created"]["username"] = self.obj["updated"]["username"] = (
            self.pfsense.get_username()
        )
    ...
    def _copy_and_update_target(self):
        ...
        if (
            "associated-rule-id" not in self.obj
            and "associated-rule-id" in before
            and before["associated-rule-id"] != ""
        ):
            self.module.fail_json(
                msg="Target filter rule is associated with a NAT rule."
            )
        ...
        if changed:
            ...
            if updated_elt is None:
                ...
                updated_elt.append(
                    self.pfsense.new_element("username", self.pfsense.get_username())
                )
    ...
    def _find_matching_rule(self):
        ...
        if "associated-rule-id" in self.obj:
            found, i = self._find_rule(
                self.obj["associated-rule-id"], "associated-rule-id"
            )
    ...
    def _find_rule(self, value, field="descr"):
        ...
        for rule_elt in self.root_elt:
            ...
            if (
                self._match_interface(rule_elt)
                and field_elt is not None
                and field_elt.text == value
            ):
                return (rule_elt, i)
            i += 1
        return (None, -1)
    ...
    def _find_target(self):
        ...
        if rule_elt is not None and self._floating:
            ...
            self._floating_interfaces = ",".join(
                [
                    self.pfsense.get_interface_display_name(interface)
                    for interface in ifs_elt.text.split(",")
                ]
            )
    ...
    def _get_expected_rule_position(self):
        ...
        if self._before == "bottom":
            return (
                self.pfsense.get_interface_rules_count(
                    self.obj["interface"], self._floating
                )
                - 1
            )
            ...
            if position is not None:
                return position
            return self.pfsense.get_interface_rules_count(
                self.obj["interface"], self._floating
            )
        return -1
    ...
    def _get_expected_rule_xml_index(self):
        ...
        if self._before == "bottom":
            ...
            if found is not None:
                ...
                self.module.fail_json(
                    msg="Failed to insert after rule=%s interface=%s"
                    % (self._after, self._interface_name())
                )
        elif self._before is not None:
            ...
            if found is not None:
                ...
                self.module.fail_json(
                    msg="Failed to insert before rule=%s interface=%s"
                    % (self._before, self._interface_name())
                )
        else:
            found, i = self._find_rule(self.obj["descr"])
            if found is not None:
    ...
    def _get_rule_position(self, descr=None, fail=True, first=True):
        ...
        res = self.pfsense.get_rule_position(
            descr, self.obj["interface"], self._floating, first=first
        )
        if fail and res is None:
            self.module.fail_json(
                msg="Failed to find rule=%s interface=%s"
                % (descr, self._interface_name())
            )
        return res
    ...
    def _insert(self, rule_elt):
        ...
        self._adjust_separators(
            rule_position, before=(self._after is None and self._before is not None)
        )

    def _match_interface(self, rule_elt):
        """check if a rule elt match the targeted interface"""
        return self.pfsense.rule_match_interface(
            rule_elt, self.obj["interface"], self._floating
        )
    ...
    def _rule_element_to_dict(self):
        ...
        for addr_item in ["source", "destination"]:
            rule[addr_item], rule[addr_item + "_port"] = self.pfsense.addr_normalize(
                rule[addr_item]
            )
...
if (filter_configure() == 0) { clear_subsystem_dirty('filter'); }"""
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "tcpflags_any", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "statetype", default="keep state"
            )
            ...
            values += self.format_cli_field(
                self.params, "log", fvalue=self.fvalue_bool, default=False
            )
            ...
            values += self.format_cli_field(
                self.params, "quick", fvalue=self.fvalue_bool, default=False
            )
        else:
            ...
            values += self.format_updated_cli_field(
                fafter, fbefore, "source_port", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                fafter, fbefore, "destination", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                fafter, fbefore, "destination_port", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "protocol", none_value="'any'", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "icmptype", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                fafter, fbefore, "interface", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "floating", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "direction", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ipprotocol", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "statetype", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "action", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "log", fvalue=self.fvalue_bool, add_comma=(values)
            )
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "defaultqueue", fname="queue", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ackqueue", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "dnpipe", fname="in_queue", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "pdnpipe", fname="out_queue", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "gateway", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "tracker", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "sched", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "quick", fvalue=self.fvalue_bool, add_comma=(values)
            )
        return values
            ...
            if "address" in rule[addr]:
                ...
                if rule[addr]["network"].endswith("ip"):
                    interface = self.pfsense.get_interface_display_name(
                        rule[addr]["network"][:-2], return_none=True
                    )
    ...
    def _obj_to_log_fields(self, rule):
        ...
        res = {}
        res["source"], res["source_port"] = self._obj_address_to_log_field(
            rule, "source"
        )
        res["destination"], res["destination_port"] = self._obj_address_to_log_field(
            rule, "destination"
        )
        res["interface"] = ",".join(
            [
                self.pfsense.get_interface_display_name(interface)
                for interface in rule["interface"].split(",")
            ]
        )
        return res
