...
class PFSenseAliasModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseAliasModule, self).__init__(
            module,
            pfsense,
            root="aliases",
            node="alias",
            key="name",
            update_php=ALIAS_PHP_COMMAND_SET,
            map_param_if=ALIAS_MAP_PARAM_IF,
            create_default=ALIAS_CREATE_DEFAULT,
        )
    ...
    def _validate_params(self):
        ...
        if params["state"] == "present":
            ...
            if alias_elt is not None:
                if params["type"] not in ["host", "network"] or alias_elt.find(
                    "type"
                ).text not in ["host", "network"]:
                    if params["type"] != alias_elt.find("type").text:
                        self.module.fail_json(
                            msg="An alias with this name and a different type already exists: '{0}'".format(
                                params["name"]
                            )
                        )
            ...
            if self.pfsense.get_interface_by_display_name(params["name"]) is not None:
                self.module.fail_json(
                    msg="An interface description with this name already exists: '{0}'".format(
                        params["name"]
                    )
                )
            ...
            if (
                params["updatefreq"] is not None
                and params["type"] != "urltable"
                and params["type"] != "urltable_ports"
            ):
                self.module.fail_json(
            ...
            details = (
                params["detail"].split("||") if params["detail"] is not None else []
            )
            if params["address"] is not None:
                ...
                if len(details) > len(addresses):
                    self.module.fail_json(
                        msg="Too many details in relation to addresses"
                    )

                # warn if address is used with urltable to urltable_ports
                if params["type"] in ["urltable", "urltable_ports"]:
                    self.module.warn(
                        'Use of "address" with {type} is depracated, please use "url" instead'.format(
                            type=params["type"]
                        )
                    )
