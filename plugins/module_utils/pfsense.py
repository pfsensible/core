...
class PFSenseModule(object):
    ...
    def find_elt(
        self, node, search_text, search_field="descr", root_elt=None, multiple_ok=False
    ):
        ...
        if multiple_ok:
            ...
            if len(result) == 1:
                ...
                self.module.fail_json(
                    msg="Found multiple elements for name {0}.".format(self.obj["name"])
                )
        return None
    ...
    def rule_match_interface(rule_elt, interface, floating):
        ...
        if floating_elt is not None:
            ...
            return False
        return (
            interface_elt is not None
            and interface_elt.text.lower() == interface.lower()
        )
    ...
    def copy_dict_to_element(self, src, top_elt, sub=0, prev_elt=None):
        ...
        for key, value in src.items():
            ...
            self.debug.write(
                "changed=%s key=%s value=%s this_elt=%s, sub=%d\n"
            ...
            if this_elt is None:
                if isinstance(value, dict):
                    ...
                    if prev_elt is not None:
                        prev_elt.tail = "\n%s" % ("\t" * (sub + 2))
                    prev_elt = new_elt
                    self.copy_dict_to_element(
                        value, new_elt, sub=sub + 1, prev_elt=prev_elt
                    )
                    ...
                    if value:
                        ...
                        for item in value:
                            ...
                            if isinstance(item, dict):
                                self.copy_dict_to_element(
                                    item, new_elt, sub=sub + 1, prev_elt=prev_elt
                                )
                            else:
                                new_elt.text = item
                            top_elt.append(new_elt)
                    ...
                    top_elt.append(new_elt)
                self.debug.write(
                    "changed=%s added key=%s value=%s tag=%s\n"
                    % (changed, key, value, top_elt.tag)
                )
            else:
                if isinstance(value, dict):
                    self.debug.write("calling copy_dict_to_element()\n")
                    if self.copy_dict_to_element(
                        value, this_elt, sub=sub + 1, prev_elt=this_elt
                    ):
                        changed = True
                elif isinstance(value, list):
                    ...
                    for idx, item in enumerate(value):
                        if isinstance(item, str):
                            if all_sub_elts[idx].text is None and item == "":
                                ...
                                changed = True
                        elif self.copy_dict_to_element(
                            item, all_sub_elts[idx], sub=sub + 1, prev_elt=prev_elt
                        ):
                            changed = True
                    ...
                    changed = True
                self.debug.write(
                    "changed=%s this_elt.text=%s value=%s\n"
    ...
    def find_alias(self, name, aliastype=None):
        ...
        for alias in self.aliases:
            if xml_find(alias, "name").text == name and (
                aliastype is None or xml_find(alias, "type").text == aliastype
            ):
                return alias
        return None
    ...
    def is_ip_or_alias(self, address):
        ...
        # Is it an alias?
        if (
            self.find_alias(address, "host") is not None
        ...
        if (
            self.is_ipv4_address(address)
            or self.is_ipv4_network(address)
            or self.is_ipv6_address(address)
            or self.is_ipv6_network(address)
        ):
            return True
    ...
    def is_gateway_group(self, gwgroup):
        ...
        return (
            self.find_elt_xpath(f"./gateways/gateway_group[name='{gwgroup}']")
            is not None
        )
    ...
    def is_port_or_alias(self, port):
        ...
        if (
            self.find_alias(port, "port") is not None
    ...
    def find_vlan(self, interface, tag):
        ...
        if self.vlans is not None:
            for vlan in self.vlans:
                if (
                    xml_find(vlan, "if").text == interface
                    and xml_find(vlan, "tag").text == tag
                ):
                    return vlan
    ...
    def find_gateway_elt(
        self, name, interface=None, protocol=None, dhcp=False, vti=False
    ):
        ...
        for gw_elt in self.gateways:
            ...
            if (
                ...
                and xml_find(gw_elt, "interface").text != interface
            ):
                continue
        ...
        for interface_elt in self.interfaces:
            ...
            if (
                vti
                and (protocol is None or protocol == "inet")
                and if_elt.text.startswith("ipsec")
                and descr_text + "_VTIV4" == name
            ):
                return self._create_gw_elt(name, interface_elt.tag, "inet")

            if (
                vti
                and (protocol is None or protocol == "inet6")
                and if_elt.text.startswith("ipsec")
                and descr_text + "_VTIV6" == name
            ):
            ...
            if dhcp:
                ...
                if (
                    (protocol is None or protocol == "inet")
                    and ipaddr_elt is not None
                    and ipaddr_elt.text == "dhcp"
                    and descr_text + "_DHCP" == name
                ):
                    return self._create_gw_elt(name, interface_elt.tag, "inet")

                ipaddr_elt = interface_elt.find("ipaddrv6")
                if (
                    (protocol is None or protocol == "inet6")
                    and ipaddr_elt is not None
                    and ipaddr_elt.text == "dhcp6"
                    and descr_text + "_DHCP6" == name
                ):
                    return self._create_gw_elt(name, interface_elt.tag, "inet6")
    ...
    def uniqid(prefix="", more_entropy=False):
        ...
        if more_entropy:
            return prefix + "{0:x}{1:05x}{2:.8F}".format(
                int(time.time()),
                int(time.time() * 1000000) % 0x100000,
                random.random() * 10,
            )

        return prefix + "{0:x}{1:05x}".format(
            int(time.time()), int(time.time() * 1000000) % 0x100000
        )
    ...
    def php(self, command):
        ...
        cmd += command
        cmd += "\n?>\n"
        (dummy, stdout, stderr) = self.module.run_command(
            "/usr/local/bin/php", data=cmd
        )
        # If /var/run/booting is in place, various requires will emit a "."
        (stdout, nsubs) = re.subn(r"^\.+", "", stdout)
        if nsubs > 0:
            self.module.warn(
                "/var/run/booting appears to be present, confirm successful boot and remove if appropriate."
            )
    ...
    def write_config(self, descr="Updated by ansible pfsense module"):
        ...
        if sys.version_info >= (3, 4):
            self.tree.write(
                tmp_name, xml_declaration=True, method="xml", short_empty_elements=False
            )
        else:
    ...
    def is_version(self, version, or_more=True):
        ...
        if self.pfsense_version is None:
            ...
            if match is None:
                self.module.fail_json(
                    msg="Unable to get version from pfSense (got '{0}')".format(
                        pfsense_version
                    )
                )
        ...
        for idx, ver in enumerate(version):
            ...
            if (
                ver < self.pfsense_version[idx]
                and not or_more
                or ver > self.pfsense_version[idx]
            ):
                return False
