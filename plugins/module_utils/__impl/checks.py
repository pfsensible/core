...
def check_name(self, name, objtype):
    ...
    if len(name) >= 32 or len(re.findall(r"(^_*$|^\d*$|[^a-zA-Z0-9_])", name)) > 0:
        ...
        msg = "The {0} name must not be either of the reserved words 'port' or 'pass'".format(
            objtype
        )
    else:
        try:
            socket.getprotobyname(name)
            msg = "The {0} name must not be an IP protocol name such as TCP, UDP, ICMP etc.".format(
                objtype
            )
        except socket.error:
            pass
        ...
        try:
            ...
            msg = "The {0} name must not be a well-known or registered TCP or UDP port name such as ssh, smtp, pop3, tftp, http, openvpn etc.".format(
                objtype
            )
        except socket.error:
            pass
...
def check_ip_address(
    self, address, ipprotocol, objtype, allow_networks=False, fail_ifnotip=False
):
    ...
    if ipprotocol == "inet":
        ...
        if ipv4 or ipv6:
            self.module.fail_json(
                msg="IPv4 and IPv6 addresses can not be used in objects that apply to both IPv4 and IPv6 (except within an alias)."
            )
...
def validate_openvpn_tunnel_network(self, network, ipproto):
    ...
    if network is not None and network != "":
        ...
        if alias_elt is not None:
            ...
            if len(networks) > 1:
                self.module.fail_json(
                    "The alias {0} contains more than one network".format(network)
                )
            network = networks[0]
...
def validate_string(self, name, objtype):
    ...
    if len(re.findall(r"[\000-\010\013\014\016-\037]", name)) > 0:
        self.module.fail_json(
            "The {0} name contains invalid characters.".format(objtype)
        )
