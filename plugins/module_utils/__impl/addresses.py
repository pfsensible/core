...
def is_ipv4_network(address, strict=True):
    ...
    try:
        addr = ip_network("{0}".format(address), strict=strict)
...
def is_ip_network(self, address, strict=True):
    ...
    return self.is_ipv4_network(address, strict) or self.is_ipv6_network(
        address, strict
    )
...
def is_within_local_networks(self, address):
    ...
    for network in networks:
        try:
            net = ip_network("{0}".format(network), strict=False)
            if addr in net:
                return True
        except ValueError:
            pass
...
def parse_port(self, src_ports, ret):
    ...
    if len(ports) > 1:
        if not self.is_port_or_alias(ports[1]):
            self.module.fail_json(
                msg="Cannot parse port %s, not port number or alias" % (ports[1])
            )
        ret["port"] += "-" + ports[1]
