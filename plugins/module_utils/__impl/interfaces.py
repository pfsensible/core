...
def get_interfaces_networks(self):
    ...
    for interface in self.interfaces:
        ...
        if (
            ipaddr_elt is not None
            and subnet_elt is not None
            and ipaddr_elt.text is not None
            and subnet_elt.text is not None
        ):
...
def parse_interface(self, interface, fail=True, with_virtual=True, with_gwgroup=False):
    ...
    if (
        with_virtual
        and (interface == "enc0" or interface.lower() == "ipsec")
        and self.is_ipsec_enabled()
    ):
        return "enc0"
    if (
        with_virtual
        and (interface == "openvpn" or interface.lower() == "openvpn")
        and self.is_openvpn_enabled()
    ):
        return "openvpn"
