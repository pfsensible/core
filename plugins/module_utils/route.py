...
class PFSenseRouteModule(PFSenseModuleBase):
        ...
        if target_elt is not None:
            ...
            if params["state"] == "absent" or old_network != params["network"]:
                ...
                for network in networks:
                    ...
                    self.route_cmd.append(
                        "/sbin/route delete {0} {1}".format(family, network)
                    )
    ...
    def _validate_params(self):
        ...
        if params["state"] == "present":
            gw_elt = self.pfsense.find_gateway_elt(
                params["gateway"], dhcp=True, vti=True
            )
            if gw_elt is None:
                self.module.fail_json(
                    msg="The gateway {0} does not exist".format(params["gateway"])
                )

            if (
                self.pfsense.is_ipv4_address(params["network"])
                and gw_elt.find("ipprotocol").text == "inet6"
                or self.pfsense.is_ipv6_address(params["network"])
                and gw_elt.find("ipprotocol").text == "inet"
            ):
                msg = 'The gateway "{0}" is a different Address Family than network "{1}".'.format(
                    gw_elt.find("gateway").text, params["network"]
                )
                self.module.fail_json(msg=msg)

            if (
                not self.pfsense.is_ip_network(params["network"], False)
                and self.pfsense.find_alias(params["network"], aliastype="host") is None
                and self.pfsense.find_alias(params["network"], aliastype="network")
                is None
            ):
                self.module.fail_json(
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "network", add_comma=(values)
            )
            values += self.format_updated_cli_field(
