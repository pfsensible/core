...
class PFSenseHaproxyBackendServerModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        ...
        self.backends = (
            self.haproxy.find("ha_pools") if self.haproxy is not None else None
        )
        if self.backends is None:
            self.module.fail_json(
                msg="Unable to find backends XML configuration entry. Are you sure haproxy is installed ?"
            )
    ...
    def _params_to_obj(self):
        ...
        if params["state"] == "present":
            ...
            if "ca" in params and params["ca"] is not None and params["ca"] != "":
                ...
                if ca_elt is None:
                    self.module.fail_json(
                        msg="%s is not a valid certificate authority" % (params["ca"])
                    )
            ...
            if (
                "clientcert" in params
                and params["clientcert"] is not None
                and params["clientcert"] != ""
            ):
                cert = self.pfsense.find_cert_elt(params["clientcert"])
                if cert is None:
                    self.module.fail_json(
                        msg="%s is not a valid certificate" % (params["clientcert"])
                    )
    ...
    def _validate_params(self):
        ...
        if self.backend is None:
            self.module.fail_json(
                msg="The backend named '{0}' does not exist".format(params["backend"])
            )
        ...
        if "forwardto" in params and params["forwardto"] is not None:
            ...
            if frontend_elt is None:
                self.module.fail_json(
                    msg="The frontend named '{0}' does not exist".format(
                        params["forwardto"]
                    )
                )
...
$result = haproxy_check_and_run($savemsg, true); if ($result) unlink_if_exists($d_haproxyconfdirty_path);"""
    ...
    def _get_ref_names(self, before):
        ...
        if (
            "ssl-server-ca" in before
            and before["ssl-server-ca"] is not None
            and before["ssl-server-ca"] != ""
        ):
        ...
        if (
            "ssl-server-crl" in before
            and before["ssl-server-crl"] is not None
            and before["ssl-server-crl"] != ""
        ):
        ...
        if (
            "ssl-server-clientcert" in before
            and before["ssl-server-clientcert"] is not None
            and before["ssl-server-clientcert"] != ""
        ):
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "checkssl", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(self.params, "weight")
            values += self.format_cli_field(
                self.params, "sslserververify", fvalue=self.fvalue_bool
            )
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "status", add_comma=(values), fname="mode"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "forwardto", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "address", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "port", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ssl", add_comma=(values), fvalue=self.fvalue_bool
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "weight", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "verifyhost", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "ca", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "crl", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.params, before, "clientcert", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "cookie", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "maxconn", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "advanced", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "istemplate", add_comma=(values)
            )
        return values
