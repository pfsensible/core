...
class PFSenseInterfaceModule(PFSenseModuleBase):
    ...
    def _check_overlaps(self, ipfield, netfield):
        ...
        our_addr = ip_network(
            "{0}/{1}".format(self.obj[ipfield], self.obj[netfield]), strict=False
        )
        ...
        for iface in self.root_elt:
            ...
            if (
                ipaddr_elt is None
                or subnet_elt is None
                or ipaddr_elt.text in ["dhcp", None]
                or ipaddr_elt.text in ["dhcpv6", None]
            ):
                continue

            other_addr = ip_network(
                "{0}/{1}".format(ipaddr_elt.text, subnet_elt.text), strict=False
            )
    ...
    def _params_to_obj(self):
        ...
        if params["state"] == "present":
            ...
            self._get_ansible_param(
                obj, "speed_duplex", fname="media", exclude="autoselect"
            )
            ...
            # check gateways
            if self.obj.get("gateway") and not self.pfsense.find_gateway_elt(
                self.obj["gateway"], self.target_elt.tag, "inet"
            ):
                self.module.fail_json(
                    msg="Gateway {0} does not exist on {1}".format(
                        self.obj["gateway"], self.obj["descr"]
                    )
                )

            if self.obj.get("gatewayv6") and not self.pfsense.find_gateway_elt(
                self.obj["gatewayv6"], self.target_elt.tag, "inet6"
            ):
                self.module.fail_json(
                    msg="Gateway {0} does not exist on {1}".format(
                        self.obj["gatewayv6"], self.obj["descr"]
                    )
                )
    ...
    def _validate_params(self):
        ...
        if re.match("^[a-zA-Z0-9_]+$", params["descr"]) is None:
            self.module.fail_json(
                msg='The name of the interface may only consist of the characters "a-z, A-Z, 0-9 and _"'
            )

        if params["state"] == "present":
            if (
                params.get("mac")
                and re.match("^([0-9A-F]{2}[:-]){5}([0-9A-F]{2})$", params["mac"])
                is None
            ):
                self.module.fail_json(
                    msg="MAC address must be in the following format: xx:xx:xx:xx:xx:xx (or blank)."
                )

            # todo can't change mac address on vlan interface

            if (
                params.get("ipv4_prefixlen") is not None
                and params["ipv4_prefixlen"] < 1
                or params["ipv4_prefixlen"] > 32
            ):
                self.module.fail_json(msg="ipv4_prefixlen must be between 1 and 32.")

            if (
                params.get("ipv6_prefixlen") is not None
                and params["ipv6_prefixlen"] < 1
                or params["ipv6_prefixlen"] > 128
            ):
            ...
            if params.get("interface") is not None:
                if params["interface"] not in interfaces.keys():
                    self.module.fail_json(
                        msg="{0} can't be assigned. Interface may only be one the following: {1}".format(
                            params["interface"], list(interfaces.keys())
                        )
                    )
                ...
                for interface, attributes in interfaces.items():
                    if (
                        "descr" in attributes
                        and attributes["descr"] == params["interface_descr"]
                    ):
                        if params.get("interface") is not None:
                            self.module.fail_json(
                                msg='Multiple interfaces found for "{0}"'.format(
                                    params["interface_descr"]
                                )
                            )
                        else:
                ...
                self.module.fail_json(
                    msg="one of the following is required: interface, interface_descr"
                )
            ...
            if params.get("speed_duplex") and params["speed_duplex"] not in media_modes:
                self.module.fail_json(
                    msg="For this interface, media mode may only be one the following: {0}".format(
                        media_modes
                    )
                )

            if params["ipv4_type"] == "static":
                if params.get("ipv4_address") and not self.pfsense.is_ipv4_address(
                    params["ipv4_address"]
                ):
                    self.module.fail_json(
                        msg="{0} is not a valid IPv4 address".format(
                            params["ipv4_address"]
                        )
                    )

            if params["ipv6_type"] == "static":
                if params.get("ipv6_address") and not self.pfsense.is_ipv6_address(
                    params["ipv6_address"]
                ):
                    self.module.fail_json(
                        msg="{0} is not a valid IPv6 address".format(
                            params["ipv6_address"]
                        )
                    )
    ...
    def _copy_and_update_target(self):
        ...
        if changed:
            if self.params["enable"]:
                self.setup_interface_cmds += (
                    "interface_bring_down('{0}', false);\n".format(self.target_elt.tag)
                )
                self.setup_interface_cmds += (
                    "interface_configure('{0}', true);\n".format(self.target_elt.tag)
                )
            else:
                self.setup_interface_cmds += (
                    "interface_bring_down('{0}', true);\n".format(self.target_elt.tag)
                )
    ...
    def _get_interface_elt_by_port_and_display_name(self, interface_port, name):
        ...
        for iface in self.root_elt:
            ...
            if descr_elt is None:
                continue
            if (
                iface.find("if").text.strip() == interface_port
                and descr_elt.text.strip().lower() == name.lower()
            ):
                return iface
        return None
    ...
    def _find_matching_interface(self):
        ...
        interface_elt = self._get_interface_elt_by_port_and_display_name(
            self.obj["if"], self.obj["descr"]
        )
        ...
        if interface_elt is not None:
            ...
            if used_by is not None:
                self.module.fail_json(
                    msg="Port {0} is already in use on interface {1}".format(
                        self.obj["if"], used_by
                    )
                )
            return interface_elt
            ...
            for ifgroup_elt in self.pfsense.ifgroups.findall("ifgroupentry"):
                if ifgroup_elt.find("members") is not None:
                    ...
                    if ifname in members:
                        self.module.fail_json(
                            msg="The interface is part of the group {0}. Please remove it from the group first.".format(
            ...
            if rule_elt.find("floating") is not None:
                ...
                if interface in interfaces:
                    if len(interfaces) > 1:
                        ...
                        new_ifs = ",".join(
                            [
                                self.pfsense.get_interface_display_name(new_interface)
    ...
    def _remove_all_separators(self, interface):
        ...
        for interface_elt in separators:
            ...
            for separator_elt in todel:
                cmd = "delete rule_separator '{0}', interface='{1}'".format(
                    separator_elt.find("text").text, interface
                )
                self.result["commands"].append(cmd)
        ...
        if before is None:
            ...
            if self.obj.get("ipaddrv6") != "slaac":
                values += self.format_cli_field(
                    self.obj, "ipaddrv6", fname="ipv6_address"
                )
            values += self.format_cli_field(
                self.obj, "subnetv6", fname="ipv6_prefixlen"
            )
            ...
            values += self.format_cli_field(
                self.obj, "blockpriv", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.obj, "blockbogons", fvalue=self.fvalue_bool
            )
            values += self.format_cli_field(
                self.params, "speed_duplex", fname="speed_duplex", default="autoselect"
            )
        else:
            # todo: - detect before ipv4_type for proper logging
            values += self.format_updated_cli_field(
                self.obj, before, "descr", add_comma=(values), fname="interface"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "if", add_comma=(values), fname="port"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "enable", add_comma=(values), fvalue=self.fvalue_bool
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ipv4_type", add_comma=(values), log_none="True"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ipaddr", add_comma=(values), fname="ipv4_address"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "subnet", add_comma=(values), fname="ipv4_prefixlen"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "gateway", add_comma=(values), fname="ipv4_gateway"
            )
            if (
                self.obj.get("ipaddrv6") == "slaac"
                and before.get("ipaddrv6") != "slaac"
            ):
                res = "ipv6_type=slaac"
                if values:
                    values += ", " + res
                ...
                values += self.format_updated_cli_field(
                    self.obj, before, "ipv6_type", add_comma=(values), log_none="True"
                )
                values += self.format_updated_cli_field(
                    self.obj,
                    before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "subnetv6", add_comma=(values), fname="ipv6_prefixlen"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "gatewayv6", add_comma=(values), fname="ipv6_gateway"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "spoofmac", add_comma=(values), fname="mac"
            )
            values += self.format_updated_cli_field(
                self.obj, before, "mtu", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "mss", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "media", add_comma=(values), fname="speed_duplex"
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
