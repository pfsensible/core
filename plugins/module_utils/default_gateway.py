...
class PFSenseDefaultGatewayModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseDefaultGatewayModule, self).__init__(
            module, pfsense, root="gateways"
        )
        ...
        if params["gateway"]:
            ...
            if params["ipprotocol"] == "inet":
                ...
                self.module.fail_json(
                    msg="Please specify a valid ipprotocol (inet/inet6)"
                )
    ...
    def _validate_params(self):
        ...
        """
        params = self.params
        gateway_list = ["none", "automatic"] + [
            gw["Name"] for gw in self.pfsense.find_active_gateways()
        ]
        ...
        if params["gateway"]:
            if str(params["gateway"]) not in gateway_list:
                self.module.fail_json(
                    msg="Unknown gateway %s : %s" % (params["gateway"], gateway_list)
                )
    ...
    def _log_fields(self, before=None):
        ...
        if self.params["ipprotocol"] == "inet":
            values += self.format_updated_cli_field(
                self.obj, before, "defaultgw4", add_comma=values
            )
        elif self.params["ipprotocol"] == "inet6":
            values += self.format_updated_cli_field(
                self.obj, before, "defaultgw6", add_comma=values
            )
