...
class PFSenseNatPortForwardModule(PFSenseModuleBase):
    ...
    def _params_to_obj(self):
        ...
        if self.params["state"] == "present":
            ...
            if self.params["associated_rule"] == "pass":
                ...
                self.params["associated_rule"] == "unassociated"
                and self._find_target() is not None
            ):
                self.module.fail_json(
                    msg="You cannot set an unassociated filter rule if the NAT rule already exists."
                )
            else:
            ...
            obj["source"] = self.pfsense.parse_address(
                self.params["source"], allow_self=False
            )
    ...
    def _parse_target_address(self, obj):
        ...
        if self.pfsense.find_alias(
            address, "host"
        ) is not None or self.pfsense.is_ipv4_address(address):
            obj["target"] = address
        else:
            self.module.fail_json(
                msg='"%s" is not a valid redirect target IP address or host alias.'
                % (param)
            )
        ...
        if ports is None:
            if self.params["protocol"] in ["tcp", "udp", "tcp/udp"]:
                self.module.fail_json(
                    msg='Must specify a target port with protocol "{0}".'.format(
        ...
        if ports is not None:
            if self.params["protocol"] not in ["tcp", "udp", "tcp/udp"]:
                self.module.fail_json(
                    msg='Cannot specify a target port with protocol "{0}".'.format(
                        self.params["protocol"]
                    )
                )
                ...
                self.module.fail_json(
                    msg='"{0}" is not a valid redirect target port. It must be a port alias or integer between 1 and 65535.'.format(
                        ports
                    )
                )
    ...
    def _set_associated_rule(self, before=None):
        ...
        if before is None:
            if (
                self.params["associated_rule"] == "associated"
                or self.params["associated_rule"] == "unassociated"
            ):
...
if (filter_configure() == 0) { clear_subsystem_dirty('natconf'); clear_subsystem_dirty('filter'); }"""
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "nordr", fvalue=self.fvalue_bool, default=False
            )
            values += self.format_cli_field(self.params, "interface")
            if self.pfsense.is_at_least_2_5_0():
                values += self.format_cli_field(
                    self.params, "ipprotocol", default="inet"
                )
            ...
            values += self.format_cli_field(
                self.params, "natreflection", default="system-default"
            )
            values += self.format_cli_field(
                self.params, "associated_rule", default="associated"
            )
            values += self.format_cli_field(
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "protocol", fvalue=self.fprotocol, add_comma=(values)
            )
            values += self.format_updated_cli_field(
            ...
            values += self.format_updated_cli_field(
                fafter, fbefore, "target", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
