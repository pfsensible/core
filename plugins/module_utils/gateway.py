...
class PFSenseGatewayModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseGatewayModule, self).__init__(
            module,
            pfsense,
            root="gateways",
            create_root=True,
    ...
    def _check_gateway_groups(self):
        ...
        for elt in self.root_elt:
            if (elt.tag == "defaultgw4" or elt.tag == "defaultgw6") and (
                elt.text is not None and elt.text == self.params["name"]
            ):
                return False
    ...
    def _check_subnet(self):
        ...
        def _check_vips():
            ...
            for vip_elt in virtualips:
                if (
                    vip_elt.find("interface").text != self.interface_elt.tag
                    or vip_elt.find("mode").text != "other"
                    or vip_elt.find("type").text != "network"
                ):
                    continue

                subnet = ip_network(
                    "{0}/{1}".format(
                        vip_elt.find("subnet").text, vip_elt.find("subnet_bits").text
                    ),
                    strict=False,
                )
                if addr in subnet:
        ...
        if (
            f1_elt is None
            or f1_elt.text is None
            or f2_elt is None
            or f2_elt.text is None
        ):
            self.module.fail_json(
                msg="Cannot add {0} Gateway Address because no {0} address could be found on the interface.".format(
                    inet_type
                )
            )
        ...
        try:
            ...
            addr = ip_address("{0}".format(self.params["gateway"]))
            subnet = ip_network(
                "{0}/{1}".format(f1_elt.text, f2_elt.text), strict=False
            )
            if addr in subnet or _check_vips():
                return

            self.module.fail_json(
                msg="The gateway address {0} does not lie within one of the chosen interface's subnets.".format(
                    self.params["gateway"]
                )
            )
        except ValueError:
    ...
    def _validate_params(self):
        ...
        self.target_elt = self.pfsense.find_gateway_elt(
            params["name"], dhcp=True, vti=True
        )
        if (
            self.target_elt is not None
            and self.target_elt.find("gateway").text == "dynamic"
        ):
            self.dynamic = True

        if params["state"] == "present":
            # check weight
            if params.get("weight") is not None and (
                params["weight"] < 1 or params["weight"] > 30
            ):
...
"""
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_cli_field(
                self.params, "monitor_disable", fvalue=self.fvalue_bool, default=False
            )
            values += self.format_cli_field(
                self.params, "action_disable", fvalue=self.fvalue_bool, default=False
            )
            values += self.format_cli_field(
                self.params, "force_down", fvalue=self.fvalue_bool, default=False
            )
            values += self.format_cli_field(self.obj, "weight", default="1")
            values += self.format_cli_field(
                self.params, "nonlocalgateway", fvalue=self.fvalue_bool, default=False
            )
        else:
            fbefore = dict()
            fbefore["interface"] = self.pfsense.get_interface_display_name(
                before["interface"]
            )

            values += self.format_updated_cli_field(
                self.params, fbefore, "interface", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ipprotocol", default="inet", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "gateway", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "descr", default="", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "monitor", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "weight", default="1", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "nonlocalgateway", fvalue=self.fvalue_bool
            )
