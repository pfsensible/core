...
class PFSenseIpsecProposalModule(PFSenseModuleBase):
    ...
    def _validate_params(self):
        ...
        if (
            params["encryption"] in key_length.keys()
            and str(params["key_length"]) not in key_length[params["encryption"]]
        ):
            msg = "key_length for encryption {0} must be one of: {1}.".format(
                params["encryption"], ", ".join(key_length[params["encryption"]])
            )
        ...
        if params.get("ikeid") is not None:
            ...
            if self._phase1 is None:
                self.module.fail_json(
                    msg="No ipsec tunnel named {0}".format(params["descr"])
                )
    ...
    def _find_target(self):
        ...
        if (
            ...
            and self.params.get("prf") is None
            and self.pfsense.is_at_least_2_5_0()
        ):
