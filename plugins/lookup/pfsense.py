...
def ordered_load(stream, loader_cls=yaml.Loader, object_pairs_hook=OrderedDict):
    ...
    if DNS_IMPORT_ERROR:
        raise AnsibleError(
            "dns must be installed to use ordered_load from this plugin"
...
def is_private_ip(address):
    ...
    if res is None:
        if not isinstance(address, ipaddress.IPv4Address):
            ...
            ip_address = address
        res = (
            ip_address in is_private_ip.classA
            or ip_address in is_private_ip.classB
            or ip_address in is_private_ip.classC
        )
...
def is_private_network(address):
    ...
    if res is None:
        if not isinstance(address, ipaddress.IPv4Network):
            ...
            net = address
        res = (
            net.subnet_of(is_private_network.classA)
            or net.subnet_of(is_private_network.classB)
            or net.subnet_of(is_private_network.classC)
        )
...
@static_vars(
    ip_broadcast=ipaddress.IPv4Address(("255.255.255.255")),
    net_multicast=ipaddress.IPv4Network(("224.0.0.0", "255.255.255.0")),
    res_cache=dict(),
)
def is_ip_broadcast(address):
    ...
    if res is None:
        if not isinstance(address, ipaddress.IPv4Address):
            ...
            ip_address = address
        res = (
            ip_address == is_ip_broadcast.ip_broadcast
            or ip_address in is_ip_broadcast.net_multicast
        )
...
def resolve_hostname(address, dns_servers=None):
    ...
    if dns_servers is None:
        ...
        msg = "Unable to resolve: {0} using {1} dns servers".format(
            address, ",".join(dns_servers)
        )
...
def get_bool(values, field):
    ...
    if isinstance(values[field], bool):
        ...
        if values[field].lower() in ["yes", "true"]:
            ...
            raise AnsibleError(
                '{0} must be yes/no or true/false (got "{1}")'.format(
                    field, values[field]
                )
            )
    return field_value
...
class PFSenseHostAlias(object):
    ...
    def copy(self):
        ...
        for network in self.networks:
            ...
            new_network.network_address = ipaddress.IPv4Address.__new__(
    ...
    def compute_addresses(self, data):
        ...
        while todo:
            ...
            if address not in data.all_aliases:
                ...
                raise AssertionError(
                    "Invalid address: " + address + " for " + self.name
                )
    ...
    def routed_by_interfaces(self, pfsense, use_remote_networks=True):
        ...
        for alias_ip in self.ips:
            interfaces = pfsense.interfaces_adjacent_or_remote_networks_contains(
                alias_ip, use_remote_networks=True
            )
            interfaces = pfsense.hack_internet_routing(
                interfaces, alias_ip, use_remote_networks=True
            )
            all_interfaces.update(interfaces)

        for alias_net in self.networks:
            interfaces = pfsense.interfaces_adjacent_or_remote_networks_contains(
                alias_net, use_remote_networks=True
            )
            interfaces = pfsense.hack_internet_routing(
                interfaces, alias_net, use_remote_networks=True
            )
            all_interfaces.update(interfaces)
...
class PFSenseInterface(object):
    ...
    def _networks_contains(address, networks):
        ...
        if isinstance(address, ipaddress.IPv4Address):
            ...
            raise AssertionError(
                "wrong type in remote_networks_contains:" + type(address)
            )
        return False
        ...
        if self.local_networks:
            for local_network in self.local_networks:
                if isinstance(address, ipaddress.IPv4Address):
                    ...
                    raise AssertionError(
                        "wrong type in local_network_contains:" + type(address)
                    )
        return False
...
class PFSense(object):
    ...
    def any_network_contains(self, address):
        ...
        return (
            self.any_local_network_contains(address)
            or self.any_remote_networks_contains(address)
            or self.any_adjacent_networks_contains(address)
        )
    ...
    def _interfaces_network_contains(self, address, networks_name):
        ...
        if isinstance(address, ipaddress.IPv4Address):
            ...
            raise AssertionError(
                "wrong type in _interfaces_network_contains:" + type(address)
            )
        return res
    ...
    def interfaces_adjacent_or_remote_networks_contains(
        self, address, use_remote_networks=True
    ):
    ...
    @static_vars(internet=ipaddress.IPv4Network(("0.0.0.0", "0.0.0.0")))
...
class PFSenseDataParser(object):
    ...
    def parse_host_alias(
        self, obj, src_name, type_name, name, allow_any, dns_servers=None
    ):
        ...
        if not values:
            self._data.set_error(
                "Empty " + src_name + " field for " + type_name + " " + name
            )
            return False
        ...
        for value in values:
            if is_valid_ip(value):
                if value not in self._data.hosts_aliases_obj:
                    self._data.hosts_aliases_obj[value] = self.create_obj_host_alias(
                        value
                    )
                ip_defs = ip_defs + 1
                continue
            ...
            if value not in self._data.hosts_aliases and (
                value != "any" or not allow_any
            ):
                if is_fqdn(value):
                    if value not in self._data.hosts_aliases_obj:
                        self._data.hosts_aliases_obj[value] = (
                            self.create_obj_host_alias(value, dns_servers)
                        )
                ...
                self._data.set_error(
                    value
                    + " is not a valid alias, ip address or network in "
                    + type_name
                    + " "
                    + name
                )
        ...
        if fqdn_defs and (ip_defs + net_defs + other_defs) > 0:
            self._data.set_error(
                "fqdn definitions can't be mixed with aliases, IP or networks addresses (in "
                + type_name
                + " "
                + name
                + ")"
        ...
        if not allow_any:
            if net_defs > 0:
                if net_defs != len(values):
                    self._data.set_error(
                        "mixed network definitions and aliases or IP addresses in "
                        + type_name
                        + " "
                        + name
                    )
        ...
        for name, alias in self._data.hosts_aliases.items():
            ...
            for field in alias:
                if field not in ["ip", "host", "descr", "dns", "ignore_dup", "ignored"]:
                    self._data.set_error(
                        field + " is not a valid field name in alias " + name
                    )
                    ret = False
            ...
            if not self.parse_host_alias(
                alias, "ip", "alias", name, False, dns_servers
            ):
                ret = False
                continue
            ...
            if dup in dups:
                display.warning(
                    "duplicate alias definition for ip "
                    + alias["ip"]
                    + " ("
                    + dups[dup]
                    + ", "
    ...
    def check_port_alias(self, ports, src_name, type_name, name):
        ...
        if src_name == "dst_nat_port":
            if "-" in ports:
                self._data.set_error(
                    "There must be only one port in dst_nat_port of " + name
                )
                return False

            if len(values) > 1:
                self._data.set_error(
                    "There must be only one port in {0} of {1}".format(src_name, name)
                )
                return False
        ...
        for value in values:
            if (
                not is_valid_port(value)
                and not is_valid_port_range(value)
                and value not in self._data.ports_aliases
            ):
                self._data.set_error(
                    value
                    + " is not a valid alias, port or port range in "
                    + type_name
                    + " "
                    + name
                )
    ...
    def parse_ports_aliases(self):
        ...
        for name, alias in self._data.ports_aliases.items():
            ...
            if dup in dups:
                display.warning(
                    "duplicate alias definition for port "
                    + alias["port"]
                    + " ("
                    + dups[dup]
                    + ", "
    ...
    def create_obj_rule_from_def(self, name, rule, separator):
        ...
        if "force" in rule:
            ...
            if obj.force and not (
                obj.get_option("filter") and obj.get_option("ifilter")
            ):
                self._data.set_error(
                    "force must not be used without filter and ifilter"
                )
    ...
    def parse_rules(self, parent=None, parent_separator=None):
        ...
        for name, rule in parent.items():
            ...
            if name == "options":
                ...
                if parent_separator.options and parent_separator.options.get(
                    "invisible"
                ):
                    if parent_separator.name is None or not parent_separator.name:
            ...
            for field in ["src", "dst"]:
                ...
                if field not in rule:
                    self._data.set_error(
                        "No {0} field for rule {1}".format(field, name)
                    )
                    ret = False
                    continue
            ...
            for field in ["src_nat", "dst_nat"]:
                ...
                if len(rule[field].split()) > 1:
                    self._data.set_error(
                        "There must be only one address in {0} field of {1}".format(
                            field, name
                        )
                    )
                    ret = False
            ...
            for field in ["src_port", "dst_port", "dst_nat_port"]:
                if field in rule:
                    ...
                    if not self.check_port_alias(
                        rule[field], field, "rule", name
                    ) or not self.check_tcp_udp(rule, name):
                        ret = False

            if "dst_nat_port" in rule and "dst_nat" not in rule:
                self._data.set_error(
                    "dst_nat_port field is set on {0} without any dst_nat target".format(
                        name
                    )
                )
                ret = False
            ...
            if "dst_nat" in rule and "dst_nat_port" not in rule:
                self._data.set_error(
                    "dst_nat field is set on {0} without any dst_nat_port target".format(
                        name
                    )
                )
                ret = False
            ...
            for field in rule:
                if field not in valid_fields:
                    self._data.set_error(
                        field + " is not a valid field name in rule " + name
                    )
                    ret = False
            ...
            self._data.rules_obj[name] = self.create_obj_rule_from_def(
                name, rule, parent_separator
            )
    ...
    def parse_target_name(self):
        ...
        if self._data.target_name not in self._data.pfsenses:
            self._data.set_error(
                self._data.target_name + " does not exist in pfsenses section"
            )
            return False
    ...
    def check_tcp_udp(self, rule, name):
        ...
        for protocol in protocols:
            if protocol != "udp" and protocol != "tcp" and protocol != "tcp/udp":
                self._data.set_error(
                    protocol
                    + " protocol used with src_port or dst_port in rule "
                    + name
                )
                return False
        return True
    ...
    def create_pfsenses_aliases(self):
        ...
        def _warn_alias(alias):
            if len(alias) >= 32:
                display.warning(
                    "Autogenerated alias {0} is too long and will trigger an error if used".format(
                        alias
                    )
                )
        ...
        for name, pfsense in self._data.pfsenses.items():
            ...
            for iname, interface in pfsense["interfaces"].items():
                ...
                if interface_definition:
                    for idx, tag in enumerate(tags):
                        ...
                        if (
                            ...
                            not in all_pfsenses_interfaces_definition[ikey]
                        ):
                            all_pfsenses_interfaces_definition[ikey].append(
                                interface_definition_name
                            )
                        ...
                        interfaces_definition[interface_definition_name].extend(
                ...
                if net_interface_definition:
                    for idx, tag in enumerate(tags):
                        ...
                        if (
                            interface_definition_name
                            not in net_all_pfsenses_interfaces_definition[ikey_net]
                        ):
                            net_all_pfsenses_interfaces_definition[ikey_net].append(
                                interface_definition_name
                            )
                        ...
                        if interface_definition_name not in net_interfaces_definition:
                            net_interfaces_definition[interface_definition_name] = (
                                list()
                            )
                        net_interfaces_definition[interface_definition_name].extend(
                            net_interface_definition
                        )
            ...
            for interface_definition_name in sorted(net_interfaces_definition.keys()):
                net_interface_definition = net_interfaces_definition[
                    interface_definition_name
                ]
        ...
        for iname, interface in pfsense["interfaces"].items():
            ...
            for key in interface:
                if key not in [
                    ...
                    self._data.set_error(
                        "Invalid field " + key + " in " + iname + " of " + name
                    )
                    return {}
            ...
            if "ip" in interface:
                for ip in interface["ip"].split():
                    ...
                    if local_network.prefixlen == 32:
                        self._data.set_error(
                            "Invalid network prefix length for network "
                            + ip
                            + " in "
                            + name
                        )
    ...
    def parse_pfsenses(self):
        ...
        for name, pfsense in self._data.pfsenses.items():
            ...
            for field in pfsense:
                if field != "interfaces":
                    self._data.set_error(
                        field + " is not a valid field name in pfsense " + name
                    )
                    ret = False
            ...
            if dup in dups:
                display.warning(
                    "duplicate pfsense definition for ip "
                    + pfsense["ip"]
                    + " ("
                    + dups[dup]
                    + ", "
...
class PFSenseRuleDecomposer(object):
    ...
    def host_separate(self, host):
        ...
        if host.is_whole_not_in_pfsense(self._data.target):
            if self._data.debug is not None and self._data.debug == host.name:
                display.warning(
                    "{0}: is_whole_not_in_pfsense {1}".format(
                        host.name, self._data.target.name
                    )
                )
            ...
            if self._data.debug is not None and self._data.debug == host.name:
                display.warning(
                    "{0}: is_whole_in_pfsense {1}".format(
                        host.name, self._data.target.name
                    )
                )
            ...
            if "ip" in alias:
                for alias_ip in alias["ip"].split():
                    ...
                    if self._data.debug is not None and self._data.debug == host.name:
                        display.warning(
                            "{0}: host_separate: {1}".format(host.name, ret_n)
                        )
                    ret.extend(ret_n)
    ...
    def host_separate_by_iface(self, host):
        ...
        if host.is_whole_in_same_routing_ifaces(self._data.target):
            if self._data.debug is not None and self._data.debug == host.name:
                display.warning(
                    "{0}: is_whole_in_same_routing_ifaces {1}".format(
                        host.name, self._data.target.name
                    )
                )
            ret.append(host)
            ...
            if "ip" in alias:
                for alias_ip in alias["ip"].split():
                    ret_n = self.host_separate_by_iface(
                        self._data.hosts_aliases_obj[alias_ip]
                    )
                    if self._data.debug is not None and self._data.debug == host.name:
                        display.warning(
                            "{0}: host_separate_by_iface: {1}".format(host.name, ret_n)
                        )
                    ret.extend(ret_n)
    ...
    def decompose_rule(self, rule):
        ...
        if not sub_rules:
            sub_rules = self.separate_aliases(
                rule, src, "src", "host_separate_by_iface"
            )
        if not blocking and not sub_rules:
            sub_rules = self.separate_aliases(
                rule, dst, "dst", "host_separate_by_iface"
            )
...
class PFSenseRuleFactory(object):
    ...
    def rule_interfaces_any(self, rule_obj):
        ...
        if src.name == "any" and dst.name == "any":
            ...
            for iface, interface in self._data.target.interfaces.items():
                if dst.is_in_adjacent_networks(interface) or dst.is_in_remote_networks(
                    interface
                ):
                    return set(self._data.target.interfaces.keys())
            return set()
            ...
            for iface, interface in self._data.target.interfaces.items():
                if (
                    src.is_in_local_network(interface)
                    or src.is_in_adjacent_networks(interface)
                    or src.is_in_remote_networks(interface)
                ):
                    interfaces.add(iface)
            if self._data.debug is not None and self._data.debug == rule_obj.name:
                display.warning(
                    "{0}: to_any_dst interfaces={1}".format(rule_obj.name, interfaces)
                )
    ...
    def rule_interfaces_ip_broadcast(self, rule_obj):
        ...
        if src_is_bcast and rule_obj.dst[0].is_whole_local(self._data.target):
            return (
                rule_obj.dst[0].local_interfaces[self._data.target.name]
                | rule_obj.dst[0].routed_interfaces[self._data.target.name]
            )

        if dst_is_bcast and rule_obj.src[0].is_whole_local(self._data.target):
            return (
                rule_obj.src[0].local_interfaces[self._data.target.name]
                | rule_obj.src[0].routed_interfaces[self._data.target.name]
            )
    ...
    def rule_interfaces(self, rule_obj):
        ...
        if self._data.debug is not None and self._data.debug == rule_obj.name:
            display.warning(
                "{0}: src={1} dst={2}".format(
                    rule_obj.name, rule_obj.src[0].name, rule_obj.dst[0].name
                )
            )
        ...
        if self._data.debug is not None and self._data.debug == rule_obj.name:
            display.warning(
                "{0}: src_is_local={1} dst_is_local={2}".format(
                    rule_obj.name, src_is_local, dst_is_local
                )
            )
        ...
        if rule_obj.action != "pass":
            if src_is_local:
                ...
                interfaces = rule_obj.src[0].routed_by_interfaces(
        ...
        if src_is_local and dst_is_local:
            if len(rule_obj.src[0].local_interfaces[self._data.target.name]) != 1:
                raise AssertionError(
                    "Invalid local interfaces count for {0}: {1}".format(
                        rule_obj.name,
                        len(rule_obj.src[0].local_interfaces[self._data.target.name]),
                    )
                )
            if len(rule_obj.dst[0].local_interfaces[self._data.target.name]) != 1:
                raise AssertionError(
                    "Invalid local interfaces count for {0}: {1}".format(
                        rule_obj.name,
                        len(rule_obj.dst[0].local_interfaces[self._data.target.name]),
                    )
                )
            ...
            if (
                ...
                and not self._data.target.interfaces[src_interface].bridge
                and self._data.target.interfaces[src_interface].are_in_same_network(
                    rule_obj.src[0], rule_obj.dst[0]
                )
            ):
                if not rule_obj.src[0].match_local_interface_ip(
                    self._data.target
                ) and not rule_obj.dst[0].match_local_interface_ip(self._data.target):
                    return set()
        ...
        if (
            not dst_is_local
            and src_is_local
            and not rule_obj.dst[0].is_adjacent_or_remote(self._data.target)
        ):
            if self._display_warnings:
                display.warning(
                    "Destination {0} is not accessible from this pfSense for {1}.Please add the right adjacent/remote network if it's not an error".format(
                        rule_obj.dst[0].name, rule_obj.name
                    )
                )
            return set()
        ...
        if (
            not src_is_local
            and dst_is_local
            and not rule_obj.src[0].is_adjacent_or_remote(self._data.target)
        ):
            if self._display_warnings:
                display.warning(
                    "Source {0} can not access to this pfSense for {1}. Please add the right adjacent/remote network if it's not an error".format(
                        rule_obj.src[0].name, rule_obj.name
                    )
                )
            return set()
        ...
        if not src_is_local:
            ...
            routing_interfaces = rule_obj.src[0].routed_by_interfaces(
                self._data.target, not src_is_adjacent
            )

            if self._data.debug is not None and self._data.debug == rule_obj.name:
                display.warning(
                    "{0}: src_is_adjacent={1}, routing_interfaces={2}, src={3}".format(
                        rule_obj.name,
                        src_is_adjacent,
                        routing_interfaces,
                        rule_obj.src[0].name,
            ...
            if not dst_is_local:
                ...
                if not src_is_adjacent and dst_is_adjacent:
                    if (
                        self._data.debug is not None
                        and self._data.debug == rule_obj.name
                    ):
                        display.warning(
                            "{0}: dst_is_adjacent={1}, routing_interfaces={2}, dst={3}".format(
                                rule_obj.name,
                                dst_is_adjacent,
                                filter_interfaces(routing_interfaces),
                                rule_obj.dst[0].name,
                            )
                        )
                ...
                dst_routing_interfaces = rule_obj.dst[0].routed_by_interfaces(
                    self._data.target, src_is_adjacent
                )
                if self._data.debug is not None and self._data.debug == rule_obj.name:
                    display.warning(
                        "{0}: dst_is_adjacent={1}, dst_routing_interfaces={2}, dst={3}".format(
                            rule_obj.name,
                            dst_is_adjacent,
                            dst_routing_interfaces,
                            rule_obj.dst[0].name,
                        )
                    )
                ...
                routing_interfaces = routing_interfaces.difference(
                    dst_routing_interfaces
                )
            ...
            bridge_interfaces = self.bridged_by_interfaces(
                routing_interfaces, rule_obj.dst[0]
            )
        ...
        if not interfaces and (src_is_local or dst_is_local):
            msg = "Invalid sub-rule interfaces count ({0}), src={1}, dst={2}".format(
                len(interfaces), rule_obj.src[0].name, rule_obj.dst[0].name
            )
            raise AssertionError(msg)
        return filter_interfaces(interfaces)
    ...
    def generate_rule(self, name, rule_obj, interfaces, last_name):
        ...
        if self._data.aggregate:
            if rule_obj.src_port:
                if len(rule_obj.src_port) == 1:
                    ...
                    rule["src_port"] = self._data.get_ports_alias(
                        set(rule_obj.src_port), name
                    )
            ...
            if rule_obj.dst_port:
                if len(rule_obj.dst_port) == 1:
                    ...
                    rule["dst_port"] = self._data.get_ports_alias(
                        set(rule_obj.dst_port), name
                    )
        ...
        if rule_obj.floating:
            ...
            for interface in rule_obj.interfaces:
                if len(base) == 1:
                    if rule_obj.src_nat:
                        _gen_src_nat_rule_dict(
                            base[0], name, interface, rule_obj.src_nat[0]
                        )

                    if rule_obj.dst_nat:
                        _gen_dst_nat_rule_dict(
                            base[0],
                            name,
                            interface,
                            rule_obj.dst_nat[0],
                            rule_obj.dst_nat_port[0],
                        )
                    ...
                    for rule_def in base:
                        ...
                        if rule_obj.src_nat:
                            _gen_src_nat_rule_dict(
                                rule_def, rule_name, interface, rule_obj.src_nat[0]
                            )
    ...
    def aggregate_subrules(self, rule, interfaces, subrules, sub_interfaces):
        ...
        def _aggregate_job(interface=None):
            ...
            if len(src_group_name) != 1:
                src = self._data.get_hosts_alias(
                    src_group_name, src_group_ip, src_group_net, rule.name
                )
            elif (
                not rule.src_nat
                and not rule.floating
                and len(subrule.src[0].networks) == 1
                and len(subrule.src[0].ips) == 0
                and subrule.src[0].networks[0]
                == self._data.target.interfaces[interface].local_network
            ):
                ...
                not rule.src_nat
                and not rule.floating
                and len(subrule.src[0].networks) == 0
                and len(subrule.src[0].ips) == 1
                and subrule.src[0].ips[0]
                == self._data.target.interfaces[interface].local_ip
            ):
                src = subrule.src[0].copy()
                src.name = "IP:{0}".format(interface)

            if len(dst_group_name) != 1:
                dst = self._data.get_hosts_alias(
                    dst_group_name, dst_group_ip, dst_group_net, rule.name
                )
            elif (
                not rule.src_nat
                and not rule.floating
                and len(subrule.dst[0].networks) == 1
                and len(subrule.dst[0].ips) == 0
                and subrule.dst[0].networks[0]
                == self._data.target.interfaces[interface].local_network
            ):
                ...
                not rule.src_nat
                and not rule.floating
                and len(subrule.dst[0].networks) == 0
                and len(subrule.dst[0].ips) == 1
                and subrule.dst[0].ips[0]
                == self._data.target.interfaces[interface].local_ip
            ):
                dst = subrule.dst[0].copy()
    ...
    def guess_rules(self, rule_filter):
        ...
        for name, rule in self._data.rules_obj.items():
            ...
            for subrule in sorted(
                rule.sub_rules, key=lambda x: x.src[0].name + x.dst[0].name
            ):
        ...
        for interface in interfaces:
            for rule in rules:
                if (
                    ...
                    or interface == "floating"
                    and rule.get("floating")
                ):
                    definition = '          - { name: "%s", source: "%s", ' % (
                        rule["name"],
                        rule["source"],
                    )
                    ...
                    if "destination_port" in rule:
                        definition += 'destination_port: "{0}", '.format(
                            rule["destination_port"]
                        )

                    definition += 'interface: "{0}", action: "{1}"'.format(
                        rule["interface"], rule["action"]
                    )
        ...
        for interface in sorted(interfaces):
            for rule in rules:
                if interface == rule["interface"]:
                    ...
                    definition += 'interface: "{0}", address: "{1}"'.format(
                        rule["interface"], rule["address"]
                    )
        ...
        for interface in sorted(interfaces):
            for rule in rules:
                if interface == rule["interface"]:
                    ...
                    definition += 'destination: "{0}", '.format(rule["destination"])
                    definition += 'interface: "{0}", target: "{1}"'.format(
                        rule["interface"], rule["target"]
                    )
...
class PFSenseRuleSeparatorFactory(object):
    ...
    def _find_first_separator_rule(self, separator):
        ...
        for rule in self._data.rules_obj.values():
            for subrule in rule.sub_rules:
                if (
                    subrule.separator.name == separator.name
                    and separator.interface in subrule.generated_names
                ):
    ...
    def output_rule_separators(self, separators):
        ...
        for interface in interfaces:
            for separator in separators:
                if (
                    "interface" in separator
                    and interface == separator["interface"]
                    or interface == "floating"
                    and "floating" in separator
                ):
                    definition = '          - { name: "' + separator["name"] + '", '
                    if interface == "floating":
                        ...
                        definition += 'interface: "' + separator["interface"] + '", '
                    definition += (
                        'before: "' + separator["before"] + '", state: "present" }'
                    )
                    definitions.append(definition)
        definitions.sort()
        print("\n".join(definitions))
...
class LookupModule(LookupBase):
    ...
    def load_data(self, from_file):
        ...
        for section in ["hosts_aliases", "ports_aliases", "pfsenses", "rules"]:
            if section not in fvars:
                raise AnsibleError(
                    "Missing {0} section in {1}".format(section, from_file)
                )
    ...
    def _run(self, terms, variables, **kwargs):
        ...
        if len(terms) != 2:
            raise AnsibleError(
                "pfsensible.core.pfsense lookup requires a filename and another parameter in [aliases, rules, rule_separators, all_definitions]"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "--dont-aggregate",
        action="store_false",
        help="dont generate aliases to aggregate rules",
    )
    parser.add_argument(
        "-g",
        "--gendiff",
        action="store_true",
        help="output more suitable for diffs (debbuging)",
    )
