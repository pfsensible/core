...
PFSENSE_AUTHSERVER_LDAP_SPEC = {
    ...
    "transport": {"choices": ["tcp", "starttls", "ssl"]},
    "ca": {"default": "global", "type": "str"},
    "protver": {"default": "3", "choices": ["2", "3"]},
    "timeout": {"default": "25", "type": "str"},
    "scope": {"choices": ["one", "subtree"]},
...
AUTHSERVER_LDAP_CREATE_DEFAULT = dict(ldap_allow_unauthenticated=None)
...
class PFSenseAuthserverLDAPModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseAuthserverLDAPModule, self).__init__(
            module,
            pfsense,
            ...
            have_refid=True,
            create_default=AUTHSERVER_LDAP_CREATE_DEFAULT,
        )
    ...
    def _params_to_obj(self):
        ...
        if params["state"] == "present":
            ...
            if self.pfsense.config_version >= 20.1:
                ...
                if (
                    obj["ldap_caref"] is None
                    and obj["ldap_urltype"] != "TCP - Standard"
                ):
                    self.module.fail_json(msg="Could not find CA '%s'" % (params["ca"]))
    ...
    def _find_target(self):
        result = self.root_elt.findall(
            "authserver[name='{0}'][type='ldap']".format(self.obj["name"])
        )
        if len(result) == 1:
            return result[0]
        elif len(result) > 1:
            self.module.fail_json(
                msg="Found multiple ldap authentication servers for name {0}.".format(
                    self.obj["name"]
                )
            )
        else:
    ...
    def _update(self):
        ...
        return self.pfsense.phpshell(
            AUTHSERVER_LDAP_PHP_COMMAND.format(name=self.obj["name"])
        )
