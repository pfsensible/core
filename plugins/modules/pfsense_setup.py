...
from ansible_collections.pfsensible.core.plugins.module_utils.module_config_base import (
    PFSenseModuleConfigBase,
)
...
SETUP_ARGUMENT_SPEC = dict(
    ...
    webguihostnamemenu=dict(
        required=False, type="str", choices=["nohost", "hostonly", "fqdn"]
    ),
...
def validate_webguicss(self, webguicss):
    ...
    if webguicss.rstrip(".css") not in themes:
        raise ValueError(
            "The submitted theme '%s' could not be found. Pick a different theme."
            % webguicss
        )
...
class PFSenseSetupModule(PFSenseModuleConfigBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseSetupModule, self).__init__(
            module,
            pfsense,
            name="pfsense_setup",
            root="system",
            arg_route=SETUP_ARG_ROUTE,
            bool_style="absent/present",
            bool_values=SETUP_BOOL_VALUES,
            map_param=SETUP_MAP_PARAM,
        )
            ...
            for address in dns_addresses:
                ...
                if gw_key not in obj or gateway != obj[gw_key]:
                    ...
                    if self.pfsense.is_ipv4_address(address):
                        ...
                        self.route_cmds.append(
                            "/sbin/route delete -inet6 {0}".format(address)
                        )
    ...
    def _validate_hostname(self, hostname, name, strict=False):
        ...
        if strict:
            ...
            if groups is None:
                self.module.fail_json(
                    msg="A valid {0} is specified, but the domain name part should be omitted".format(
                        name
                    )
                )
    ...
    def _validate_params(self):
        ...
        if params.get("dashboardcolumns") is not None and (
            params["dashboardcolumns"] < 1 or params["dashboardcolumns"] > 6
        ):
            self.module.fail_json(
        ...
        if params.get("authmode") is not None:
            ...
            if value != "Local Database":
                authserver_elt = self.pfsense.find_elt(
                    "authserver", value, search_field="name", root_elt=self.root_elt
                )
                if authserver_elt is None:
                    self.module.fail_json(
                        msg="Given authserver '{0}' could not be found.".format(value)
                    )

                if (
                    params.get("shellauth") is not None
                    and params.get("shellauth") is True
                ):
                    if authserver_elt.find("type").text == "ldap":
                        # check if ldap_pam_groupdn is set
                        if (
                            authserver_elt.find("ldap_pam_groupdn") is None
                            or authserver_elt.find("ldap_pam_groupdn").text is None
                            or authserver_elt.find("ldap_pam_groupdn").text == ""
                        ):
                            self.module.fail_json(
                                msg="ldap_pam_groupdn not set for authserver '{0}'.".format(
                                    value
                                )
                            )
        ...
        if params.get("dns_addresses") is not None:
            ...
            for address in dns_addresses:
                ...
                if self.pfsense.is_ipv4_address(address):
                    ...
                    self.module.fail_json(
                        msg="A valid IP address must be specified for DNS server {0}.".format(
                            address
                        )
                    )
        ...
        if params.get("dns_gateways") is not None:
            for idx, address in enumerate(params["dns_gateways"].split(" ")):
                ...
                if self.pfsense.find_gateway_elt(address, protocol="inet") is not None:
                    if ip_types[idx] == 6:
                        self.module.fail_json(
                            msg='The IPv4 gateway "{0}" can not be specified for IPv6 DNS server "{1}".'.format(
                                address, dns_addresses[idx]
                            )
                        )
                elif (
                    self.pfsense.find_gateway_elt(address, protocol="inet6") is not None
                ):
                    if ip_types[idx] == 4:
                        self.module.fail_json(
                            msg='The IPv6 gateway "{0}" can not be specified for IPv4 DNS server "{1}".'.format(
                                address, dns_addresses[idx]
                            )
                        )
                else:
                    self.module.fail_json(
                        msg='The gateway "{0}" does not exist.'.format(address)
                    )

                if self.pfsense.is_within_local_networks(dns_addresses[idx]):
                    self.module.fail_json(
                        msg="A gateway can not be assigned to DNS '{0}' server which is on a directly connected network.".format(
                            dns_addresses[idx]
                        )
                    )
...
$retval |= system_ntp_configure();"""
        ...
        if self.params.get("dnsallowoverride") is not None:
            if (
                self.params["dnsallowoverride"]
                and "dnsallowoverride" not in self.diff["before"]
                or not self.params["dnsallowoverride"]
                and "dnsallowoverride" in self.diff["before"]
            ):
    ...
    def _log_fields(self, before=None):
        ...
        values += self.format_updated_cli_field(
            self.obj, self.diff["before"], "domain", add_comma=(values), log_none=False
        )

        values += self.format_updated_cli_field(
            obj_after, obj_before, "dns_addresses", add_comma=(values), log_none=True
        )
        values += self.format_updated_cli_field(
            obj_after, obj_before, "dns_hostnames", add_comma=(values), log_none=True
        )
        values += self.format_updated_cli_field(
            obj_after, obj_before, "dns_gateways", add_comma=(values), log_none=True
        )
        ...
        values += self.format_updated_cli_field(
            obj_after, obj_before, "webguicss", add_comma=(values), log_none=False
        )
        values += self.format_updated_cli_field(
            webgui,
            bwebgui,
        ...
        values += self.format_updated_cli_field(
            webgui, bwebgui, "webguihostnamemenu", add_comma=(values), log_none=False
        )
        values += self.format_updated_cli_field(
            webgui, bwebgui, "dashboardcolumns", add_comma=(values), log_none=False
        )
        ...
        values += self.format_updated_cli_field(
            webgui, bwebgui, "logincss", add_comma=(values), log_none=False
        )
        values += self.format_updated_cli_field(
            webgui,
            bwebgui,
    ...
    def _prepare_dns_log(obj):
        ...
        ret["webguicss"] = (
            webgui["webguicss"].replace(".css", "") if "webguicss" in webgui else None
        )
