...
class PFSenseCertModule(PFSenseModuleBase):
    ...
    def __init__(self, module, pfsense=None):
        super(PFSenseCertModule, self).__init__(
            module, pfsense, root="pfsense", node="cert"
        )
    ...
    def _validate_params(self):
                ...
                if ca is not None:
                    if ca.find("prv") is None:
                        self.module.fail_json(
                            msg="CA (%s) is not an internal CA" % params["ca"]
                        )
                else:
        ...
        if params["key"] is not None:
            ...
            if re.match(
                "^-----BEGIN ((EC|RSA) )?PRIVATE KEY-----$", lines[0]
            ) and re.match("^-----END ((EC|RSA) )?PRIVATE KEY-----$", lines[-1]):
    ...
    def _find_ca(self, caref):
        ...
        if len(result) == 1:
            ...
            if len(result) == 1:
                ...
                self.module.fail_json(
                    msg="Found multiple CAs for caref {0}.".format(caref)
                )
            else:
                return None
                ...
                return self.pfsense.phpshell(
                    ...
                    """.format(
                        ...
                        key=base64.b64decode(
                            self.target_elt.find("prv").text.encode()
                        ).decode(),
                    )
                )
            else:
