...
class PFSenseDHCPStaticModule(PFSenseModuleBase):
    ...
    def _validate_params(self):
        ...
        if (
            params["macaddr"] is not None
            and re.fullmatch(r"(?:[0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}", params["macaddr"])
            is None
        ):
        ...
        if params["netif"] is not None:
            if self.pfsense.is_interface_group(params["netif"]):
                self.module.fail_json(
                    msg="DHCP cannot be configured for interface groups"
                )
            else:
        ...
        if params["ipaddr"] is not None:
            addr = ip_address("{0}".format(params["ipaddr"]))
            if addr not in self.network:
                self.module.fail_json(
                    msg="The IP address must lie in the {0} subnet.".format(self.netif)
                )
    ...
    def _find_staticmaps(self, netif=None):
                ...
                if e.find("enable") is not None:
                    if self._is_valid_netif(e.tag):
                        if self.root_elt is not None:
                            self.module.fail_json(
                                msg="Multiple DHCP servers enabled and no netif specified"
                            )
                        self.root_elt = e
                        self.netif = e.tag
        ...
        if self.root_elt is None:
            if netif is None:
                ...
                self.module.fail_json(
                    msg="No DHCP configuration found for netif='{0}'".format(netif)
                )
    ...
    def _find_target(self):
        if self.params["name"] is not None and self.params["macaddr"] is not None:
            result = self.root_elt.findall(
                "staticmap[cid='{0}'][mac='{1}']".format(
                    self.params["name"], self.params["macaddr"]
                )
            )
        elif self.params["name"] is not None:
            result = self.root_elt.findall(
                "staticmap[cid='{0}']".format(self.params["name"])
            )
        else:
            result = self.root_elt.findall(
                "staticmap[mac='{0}']".format(self.params["macaddr"])
            )
        ...
        if len(result) == 1:
            ...
            self.module.fail_json(
                msg="Found multiple static maps for cid {0}.".format(self.obj["cid"])
            )
        else:
            return None
    ...
    def _log_fields(self, before=None):
        ...
        if before is None:
            ...
            values += self.format_updated_cli_field(
                self.obj, before, "macaddr", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj, before, "ipaddr", add_comma=(values)
            )
            values += self.format_updated_cli_field(
                self.obj,
                before,
