...
class PFSenseGroupModule(PFSenseModuleBase):
        ...
        if "gid" not in self.obj:
            ...
            if self._find_group_by_gid(self.obj["gid"]) is not None:
                self.module.fail_json(
                    msg="A different group already exists with gid {0}.".format(
                        self.obj["gid"]
                    )
                )
    ...
    def _find_target(self):
        return self.pfsense.find_elt(
            "group", self.obj["name"], search_field="name", root_elt=self.root_elt
        )

    def _find_group_by_gid(self, gid):
        return self.pfsense.find_elt(
            "group", gid, search_field="gid", root_elt=self.root_elt
        )
    ...
    def _update(self):
        if self.params["state"] == "present":
            return self.pfsense.phpshell(
                GROUP_PHP_COMMAND_SET.format(idx=self._find_this_group_index())
            )
        else:
            return self.pfsense.phpshell(
                GROUP_PHP_COMMAND_DEL.format(name=self.obj["name"])
            )
...
def main():
    module = AnsibleModule(
        argument_spec={
            ...
            "scope": {"default": "local", "choices": ["local", "remote", "system"]},
