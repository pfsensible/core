...
class TestPFSenseLookup(ModuleTestCase):
    ...
    def setUp(self):
        ...
        self.mock_get_hostname = patch(
            "ansible_collections.pfsensible.core.plugins.lookup.pfsense.LookupModule.get_hostname"
        )
        get_hostname = self.mock_get_hostname.start()
        get_hostname.return_value = "pf_test1"

        self.mock_get_definitions = patch(
            "ansible_collections.pfsensible.core.plugins.lookup.pfsense.LookupModule.get_definitions"
        )
    ...
    def build_definitions(self):
        ...
        self.interfaces = dict(
            ...
            LANA=dict(
                base="10.20.30.x", remote_base="10.120.x", adjacent_base="10.220.x"
            ),
            LANB=dict(
                base="10.20.40.x", remote_base="10.130.x", adjacent_base="10.230.x"
            ),
        )
        ...
        for name, defs in self.interfaces.items():
            ...
            if "base" in defs:
                self.definitions["pfsenses"]["pf_test1"]["interfaces"][name]["ip"] = (
                    defs["base"].replace("x", "1/24")
                )
            for param in ["remote_networks", "adjacent_networks"]:
                if param in defs:
                    self.definitions["pfsenses"]["pf_test1"]["interfaces"][name][
                        param
                    ] = defs[param]
            if "remote_base" in defs:
                self.definitions["pfsenses"]["pf_test1"]["interfaces"][name][
                    "remote_networks"
                ] = defs["remote_base"].replace("x", "0.0/16")
            if "adjacent_base" in defs:
                self.definitions["pfsenses"]["pf_test1"]["interfaces"][name][
                    "adjacent_networks"
                ] = defs["adjacent_base"].replace("x", "0.0/16")
    ...
    def gen_rule(self, src, dst, interface, action):
        ...
        if interface == "s":
            if dst == "l":
                ...
                rule["dst"] = self.interfaces["LANA"]["remote_base"].replace(
                    "x", "30.40"
                )
            elif dst == "a":
                rule["dst"] = self.interfaces["LANA"]["adjacent_base"].replace(
                    "x", "30.40"
                )
        else:
            if dst == "l":
                ...
                rule["dst"] = self.interfaces["LANB"]["remote_base"].replace(
                    "x", "30.40"
                )
            elif dst == "a":
                rule["dst"] = self.interfaces["LANB"]["adjacent_base"].replace(
                    "x", "30.40"
                )
