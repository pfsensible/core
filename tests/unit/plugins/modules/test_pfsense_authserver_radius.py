...
from ansible_collections.pfsensible.core.plugins.modules import (
...
class TestPFSenseAuthserverRADIUSModule(TestPFSenseModule):
    ...
    def check_target_elt(self, obj, target_elt):
        ...
        self.check_param_equal(
            obj, target_elt, "auth_port", xml_field="radius_auth_port", default=1812
        )
        self.check_param_equal(
            obj, target_elt, "acct_port", xml_field="radius_acct_port", default=1813
        )
        self.check_param_equal(
            obj, target_elt, "protocol", xml_field="radius_protocol", default="MSCHAPv2"
        )
        self.check_param_equal(obj, target_elt, "secret", xml_field="radius_secret")
        self.check_param_equal(
            obj, target_elt, "timeout", xml_field="radius_timeout", default=5
        )
        self.check_param_equal(
            obj,
            target_elt,
            "nasip_attribute",
            xml_field="radius_nasip_attribute",
            default="lan",
        )
    ...
    def test_authserver_delete(self):
        ...
        self.do_module_test(
            obj, command="delete authserver_radius 'DELRADIUS'", delete=True
        )
    ...
    def test_create_authserver_invalid_timeout(self):
        ...
        obj = dict(
            name="DELRADIUS", host="radius.example.com", secret="password1", timeout=0
        )
        self.do_module_test(
            obj,
            command="update authserver_radius 'DELRADIUS'",
            failed=True,
