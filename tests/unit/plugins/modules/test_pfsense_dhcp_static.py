...
from ansible_collections.pfsensible.core.plugins.modules.pfsense_dhcp_static import (
...
class TestPFSenseDHCPStaticModule(TestPFSenseModule):
    ...
    def check_target_elt(self, obj, target_elt, target_idx=-1):
        ...
        self.check_param_equal(
            obj, target_elt, "ddnsdomainkeyalgorithm", default="hmac-md5"
        )
    ...
    def get_target_elt(self, obj, absent=False, module_result=None):
        ...
        root_elt = None
        for e in dhcpd_elt:
            if "netif" not in obj or (
                module_result is not None and e.tag == module_result["netif"]
            ):
        ...
        if root_elt is not None:
            if "name" in obj and "macaddr" in obj:
                result = root_elt.findall(
                    "staticmap[cid='{0}'][mac='{1}']".format(
                        obj["name"], obj["macaddr"]
                    )
                )
            elif "name" in obj:
    ...
    def test_dhcp_static_create(self):
        ...
        obj = dict(
            name="test_entry",
            macaddr="ab:ab:ab:ab:ab:ac",
            ipaddr="10.0.0.101",
            netif="opt1",
        )
        command = "create dhcp_static 'test_entry', macaddr='ab:ab:ab:ab:ab:ac', ipaddr='10.0.0.101'"
    ...
    def test_dhcp_static_create_empty(self):
        ...
        command = "create dhcp_static 'test_entry', macaddr='ab:ab:ab:ab:ab:ac', ipaddr='10.10.0.101'"
        self.do_module_test(obj, command=command)
    ...
    def test_dhcp_static_create_arp_table_static_entry(self):
        ...
        obj = dict(
            name="test_entry",
            macaddr="ab:ab:ab:ab:ab:ab",
            ipaddr="10.0.0.101",
            netif="opt1",
            arp_table_static_entry=True,
        )
        command = "create dhcp_static 'test_entry', macaddr='ab:ab:ab:ab:ab:ab', ipaddr='10.0.0.101', arp_table_static_entry=True"
        self.do_module_test(obj, command=command)
    ...
    def test_dhcp_static_create_wrong_subnet(self):
        ...
        self.do_module_test(
            obj, failed=True, msg="The IP address must lie in the opt1 subnet."
        )
    ...
    def test_dhcp_static_create_no_netif(self):
        ...
        self.do_module_test(
            obj, failed=True, msg="Multiple DHCP servers enabled and no netif specified"
        )
    ...
    def test_dhcp_static_create_ifgroup(self):
        ...
        self.do_module_test(
            obj, failed=True, msg="DHCP cannot be configured for interface groups"
        )
