...
class TestPFSenseSetupModule(TestPFSenseModule):
    ...
    def setUp(self):
        ...
        self.mock_validate_webguicss = patch.dict(
            "ansible_collections.pfsensible.core.plugins.modules.pfsense_setup.SETUP_ARG_ROUTE",
    ...
    def check_target_elt(self, obj, target_elt):
        ...
        if obj.get("webguicss") is not None:
            self.assert_xml_elt_equal(
                webgui_elt, "webguicss", obj["webguicss"] + ".css"
            )
    ...
    def test_setup_dns_addresses(self):
        """test setup dns"""
        setup = dict(
            dns_addresses="8.8.4.4 8.8.8.8",
            dns_hostnames="acme1 acme2",
            dns_gateways="none GW_WAN",
        )
    ...
    def test_setup_dns_addresses_invalid(self):
        """test setup dns"""
        setup = dict(
            dns_addresses="8.8.4.4 8.8.8.8 256.255.254.253",
            dns_hostnames="acme1 acme2",
            dns_gateways="none GW_WAN",
        )
    ...
    def test_setup_dns_addresses_invalid_gw2(self):
        """test setup dns"""
        setup = dict(
            dns_addresses="8.8.4.4 192.168.1.1",
            dns_hostnames="acme1 acme2",
            dns_gateways="none GW_WAN",
        )
