...
class TestPFSenseNatOutboundModule(TestPFSenseModule):
    ...
    def is_ipv4_address(address):
        ...
        try:
            addr = ip_address("{0}".format(address))
    ...
    def parse_address(self, name, addr, field, invert=False):
        ...
        if parts[0] == "NET":
            ...
            if (
                field in res
                and self.is_ipv4_address(res[field])
                and res[field].find("/") == -1
            ):
                res[field] += "/32"
    ...
    def check_addr(self, params, target_elt, addr, field, port, invert=False):
        ...
        (addr_dict, port_value) = self.parse_address(
            addr, params[addr], field, invert=invert
        )
    ...
    def check_target_elt(self, obj, target_elt, target_idx=-1):
        ...
        self.check_value_equal(
            target_elt, "interface", self.unalias_interface(obj["interface"])
        )
        self.check_param_equal(
            obj, target_elt, "ipprotocol", "inet46", not_find_val="inet46"
        )
        self.check_param_equal(obj, target_elt, "protocol", "any", not_find_val="any")
        self.check_param_equal(obj, target_elt, "poolopts")
        self.check_value_equal(
            target_elt, "source_hash_key", self.md5(obj.get("source_hash_key"))
        )
    ...
    def test_nat_outbound_create_aliases(self):
        ...
        obj = dict(
            descr="https-source-rewriting",
            interface="lan",
            source="srv_admin:port_ssh",
            destination="srv_admin:port_ssh",
            address="srv_admin:port_ssh",
        )
        command = (
    ...
    def test_nat_outbound_create_with_sourcehashkey(self):
        ...
        obj = dict(
            ...
            source="any",
            destination="1.2.3.4:443",
            source_hash_key="0x12345678901234567890123456789012",
        )
    ...
    def test_nat_outbound_update_source(self):
        """test"""
        obj = dict(
            descr="one rule", interface="wan", source="(self):123", destination="any"
        )
    ...
    def test_nat_outbound_update_destination(self):
        """test"""
        obj = dict(
            descr="one rule", interface="wan", source="any", destination="1.2.3.4:555"
        )
    ...
    def test_nat_outbound_update_interface(self):
        """test"""
        obj = dict(
            descr="one rule", interface="lan_100", source="any", destination="any"
        )
    ...
    def test_nat_outbound_invalid_sourcehashkey_hex(self):
        ...
        obj = dict(
            ...
            source="any",
            destination="1.2.3.4:443",
            source_hash_key="0xg2345678901234567890123456789012",
        )
    ...
    def test_nat_outbound_invalid_sourcehashkey_len(self):
        ...
        obj = dict(
            ...
            source="any",
            destination="1.2.3.4:443",
            source_hash_key="0x1234567890123456789012345678901",
        )
        msg = 'Incorrect format for source-hash key, "0x" must be followed by exactly 32 hexadecimal characters.'
        self.do_module_test(obj, msg=msg, failed=True)
