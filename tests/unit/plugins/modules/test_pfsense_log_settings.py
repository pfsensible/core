...
class TestPFSenseLogSettingsModule(TestPFSenseModule):
                ...
                if param in obj:
                    ...
                    if param == "sourceip":
                        self.assert_xml_elt_equal(
                            target_elt,
                            xml_field,
                            interface_map.get(obj[param], obj[param]),
                        )
                    else:
                    ...
                    if param in self.defaults:
                        self.assert_xml_elt_equal(
                            target_elt, xml_field, self.defaults[param]
                        )
                    else:
                ...
                if param in obj:
                    ...
                    if param == "nologdefaultpass":
                        ...
                        self.check_param_bool(
                            obj, target_elt, param, xml_field=xml_field
                        )
                else:
                    if param in self.defaults:
    ...
    def test_syslog_sourceip_wan_ip(self):
        ...
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=True, sourceip="192.168.240.137"
        )
    ...
    def test_syslog_sourceip_lan(self):
        ...
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=True, sourceip="192.168.1.242"
        )
    ...
    def test_syslog_sourceip_lo0(self):
        """test log_settings sourceip=lan"""
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=True, sourceip="127.0.0.1"
        )
    ...
    def test_syslog_sourceip_descr(self):
        """test log_settings sourceip=lan"""
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=True, sourceip="Localhost"
        )
        command = "update log_settings syslog set enable=True, sourceip='lo0', remoteserver='1.2.3.4', logall=True"
        self.do_module_test(syslog, command=command, state=None)
    ...
    def test_syslog_sourceip_valid_vip_ip(self):
        ...
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=True, sourceip="10.255.2.254"
        )
    ...
    def test_syslog_remoteserver_hostname(self):
        ...
        syslog = dict(
            enable=True,
            remoteserver="2001:0db8:cafe:affe:0000:0000:0000:0001",
            logall=True,
        )
    ...
    def test_syslog_remoteserver_fqdn_port(self):
        ...
        syslog = dict(
            enable=True, remoteserver="logserver.example.com:514", logall=True
        )
    ...
    def test_syslog_remoteserver_ipv6_port(self):
        ...
        syslog = dict(
            enable=True,
            remoteserver="[2001:0db8:cafe:affe:0000:0000:0000:0001]:514",
            logall=True,
        )
    ...
    def test_syslog_portalauth_valid_with_logall(self):
        ...
        syslog = dict(
            enable=True, remoteserver="1.2.3.4", logall=False, portalauth=True
        )
        command = "update log_settings syslog set enable=True, remoteserver='1.2.3.4', portalauth=True"
        self.do_module_test(syslog, command=command, state=None)
