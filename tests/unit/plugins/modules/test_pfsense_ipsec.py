...
class TestPFSenseIpsecModule(TestPFSenseModule):
    ...
    def check_target_elt(self, obj, target_elt):
        ...
        self.assert_xml_elt_equal(
            target_elt, "authentication_method", obj["authentication_method"]
        )
        if obj["authentication_method"] == "rsasig":
            self.assert_xml_elt_equal(
                target_elt, "certref", self.certref(obj["certificate"])
            )
            self.assert_xml_elt_equal(
                target_elt, "caref", self.caref(obj["certificate_authority"])
            )
            ...
            self.assert_xml_elt_equal(
                target_elt, "pre-shared-key", obj["preshared_key"]
            )
    ...
    def test_ipsec_update_auth(self):
        ...
        ipsec = dict(
            ...
            certificate="webConfigurator default (5c00e5f9029df)",
    ...
    def test_ipsec_update_cert(self):
        ...
        ipsec = dict(
            descr="test_tunnel2",
            interface="lan_100",
            remote_gateway="1.2.3.6",
            iketype="ikev2",
            authentication_method="rsasig",
            certificate="webConfigurator default copy",
            certificate_authority="test ca copy",
        )
