...
class TestPFSenseAliasModule(TestPFSenseModule):
    ...
    def do_alias_creation_test(
        ...
        with set_module_args(self.args_from_var(alias)):
            ...
            if not failed:
                ...
                self.assert_xml_elt_dict(
                    "aliases",
                    dict(name=alias["name"], type=alias["type"]),
                    diff["after"],
                )
    ...
    def do_alias_deletion_test(self, alias, command=None):
        ...
        with set_module_args(self.args_from_var(alias, "absent")):
            ...
            self.assertEqual(result["diff"], diff)
            self.assert_has_xml_tag(
                "aliases", dict(name=alias["name"], type=alias["type"]), absent=True
            )
            self.assertEqual(result["commands"], [command])
    ...
    def test_host_delete(self):
        ...
        alias = dict(
            name="ad_poc1", address="192.168.1.3", descr="", type="host", detail=""
        )
    ...
    def test_port_create(self):
        ...
        alias = dict(
            name="port_proxy", address="8080 8443", descr="", type="port", detail=""
        )
    ...
    def test_urltable_create(self):
        ...
        self.do_alias_creation_test(
            alias,
            command=command,
            set_after=dict(url="http://www.acme.com"),
            unset_after=["address"],
        )
    ...
    def test_urltable_create_exclusive(self):
        ...
        self.do_alias_creation_test(
            alias, failed=True, msg="parameters are mutually exclusive: address|url"
        )
    ...
    def test_urltable_update_url(self):
        ...
        self.do_alias_update_field(
            alias,
            url="http://www.new-acme-corp.com",
            set_after=dict(url="http://www.new-acme-corp.com"),
            command=command,
        )
    ...
    def test_create_alias_duplicate(self):
        ...
        self.do_alias_creation_test(
            alias,
            failed=True,
            msg="An alias with this name and a different type already exists: 'port_ssh'",
        )
    ...
    def test_create_alias_invalid_name_interface(self):
        ...
        self.do_alias_creation_test(
            alias,
            failed=True,
            msg="An interface description with this name already exists: 'lan_100'",
        )

    def test_create_alias_invalid_updatefreq(self):
        """test creation of a new host alias with incoherent params"""
        alias = dict(
            name="adservers", address="10.0.0.1 10.0.0.2", type="host", updatefreq=10
        )
        self.do_alias_creation_test(
            alias,
            failed=True,
            msg="updatefreq is only valid with type urltable or urltable_ports",
        )
    ...
    def test_create_alias_without_address(self):
        ...
        self.do_alias_creation_test(
            alias,
            failed=True,
            msg="type is host but all of the following are missing: address",
        )
    ...
    def test_create_alias_invalid_details(self):
        ...
        self.do_alias_creation_test(
            alias, failed=True, msg="Too many details in relation to addresses"
        )
    ...
    def test_create_alias_invalid_details2(self):
        ...
        self.do_alias_creation_test(
            alias,
            failed=True,
            msg="Vertical bars (|) at start or end of descriptions not allowed",
        )

    def test_delete_inexistent_alias(self):
        """test deletion of an inexistent alias"""
        alias = dict(
            name="ad_poc12", address="192.168.1.3", descr="", type="host", detail=""
        )
        with set_module_args(self.args_from_var(alias, "absent")):
            result = self.execute_module(changed=False)
    ...
    def test_check_mode(self):
        ...
        with set_module_args(
            self.args_from_var(alias, address="192.168.1.4", _ansible_check_mode=True)
        ):
            result = self.execute_module(changed=True)
            ...
            self.assertFalse(self.load_xml_result())
            self.assertEqual(
                result["commands"], ["update alias 'ad_poc1' set address='192.168.1.4'"]
            )
    ...
    def test_urltable_required_if(self):
        ...
        with set_module_args(self.args_from_var(alias)):
            self.execute_module(
                failed=True,
                msg="type is urltable but all of the following are missing: updatefreq",
            )
    ...
    def test_urltable_ports_required_if(self):
        ...
        with set_module_args(self.args_from_var(alias)):
            self.execute_module(
                failed=True,
                msg="type is urltable_ports but all of the following are missing: updatefreq",
            )
