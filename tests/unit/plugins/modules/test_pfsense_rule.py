...
class TestPFSenseRuleModule(TestPFSenseModule):
    ...
    def get_target_elt(self, obj, absent=False, module_result=None):
        ...
        if "floating" in obj and obj["floating"] == "yes":
            return self.assert_has_xml_tag(
                "filter", dict(descr=obj["name"], floating="yes"), absent=absent
            )
        return self.assert_has_xml_tag(
            "filter", dict(descr=obj["name"], interface=obj["interface"]), absent=absent
        )
    ...
    def check_rule_idx(self, rule, target_idx):
        ...
        for rule_elt in rules_elt:
            ...
            if not floating:
                if (
                    floating_rule
                    or interface_elt is None
                    or interface_elt.text is None
                    or interface_elt.text != rule["interface"]
                ):
                    continue
            idx += 1
